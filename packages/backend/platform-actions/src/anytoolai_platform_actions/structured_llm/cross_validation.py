...
def _reject_duplicate_ids(items: Any, *, id_key: str, error_label: str) -> None:
    """Raises ActionInputValidationError on the first duplicate `id_key` value found in
    `items` (a dynamic list of action-input specs, e.g. A01 input.fields or A11
    input.criteria). Ignores non-list `items` and skips non-Mapping/non-str ids so this stays
    an input-validator concern, not a schema-shape check."""
...
class ExtractStructuredFieldsInputValidator:
    ...
    def validate(self, *, input_payload: Mapping[str, Any]) -> None:
        _reject_duplicate_ids(
            input_payload.get("fields"), id_key="name", error_label="fields[*].name"
        )
...
class CompareAndClassifyInputValidator:
    ...
    def validate(self, *, input_payload: Mapping[str, Any]) -> None:
        _reject_duplicate_ids(
            input_payload.get("criteria"), id_key="id", error_label="criteria[*].id"
        )
