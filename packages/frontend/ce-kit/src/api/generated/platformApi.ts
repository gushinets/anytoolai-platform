/**
 * This file was auto-generated by openapi-typescript.
 * Do not make direct changes to the file.
 */

export interface paths {
    "/health": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Health */
        get: operations["health_health_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/atoms": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** List Atoms */
        get: operations["list_atoms_v1_atom_lab_atoms_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/atoms/{atom_id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Get Atom */
        get: operations["get_atom_v1_atom_lab_atoms__atom_id__get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/models": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** List Models */
        get: operations["list_models_v1_atom_lab_models_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/models/refresh": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** Request Model Refresh */
        post: operations["request_model_refresh_v1_atom_lab_models_refresh_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/presets": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** List Presets */
        get: operations["list_presets_v1_atom_lab_presets_get"];
        put?: never;
        /** Create Preset */
        post: operations["create_preset_v1_atom_lab_presets_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/presets/{preset_id}/versions": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** List Preset Versions */
        get: operations["list_preset_versions_v1_atom_lab_presets__preset_id__versions_get"];
        put?: never;
        /** Create Preset Version */
        post: operations["create_preset_version_v1_atom_lab_presets__preset_id__versions_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/presets/{preset_id}/versions/{version}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Get Preset Version */
        get: operations["get_preset_version_v1_atom_lab_presets__preset_id__versions__version__get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/presets/{preset_id}/versions/{version}/export": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Export Preset Version */
        get: operations["export_preset_version_v1_atom_lab_presets__preset_id__versions__version__export_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/runs": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** List Runs */
        get: operations["list_runs_v1_atom_lab_runs_get"];
        put?: never;
        /** Start Run */
        post: operations["start_run_v1_atom_lab_runs_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/atom-lab/runs/{run_id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Get Run */
        get: operations["get_run_v1_atom_lab_runs__run_id__get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/client-events": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** Record an allowlisted client analytics event */
        post: operations["post_client_event_v1_client_events_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/demo/runs": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** Start an allowlisted stakeholder demo workflow */
        post: operations["start_demo_run_v1_demo_runs_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/handoffs": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** Create Handoff */
        post: operations["create_handoff_v1_handoffs_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/handoffs/{handoff_token}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Get Handoff */
        get: operations["get_handoff_v1_handoffs__handoff_token__get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/handoffs/{handoff_token}/accept": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** Accept Handoff */
        post: operations["accept_handoff_v1_handoffs__handoff_token__accept_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/handoffs/{handoff_token}/decline": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** Decline Handoff */
        post: operations["decline_handoff_v1_handoffs__handoff_token__decline_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/identity/guest": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** Create an opaque guest identity */
        post: operations["create_guest_identity_v1_identity_guest_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/products/{product_id}/quota": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Get frontend-safe guest quota state */
        get: operations["get_product_quota_v1_products__product_id__quota_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/products/{product_id}/runtime-config": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Get frontend-safe product runtime config */
        get: operations["get_runtime_config_v1_products__product_id__runtime_config_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/products/{product_id}/scenarios/{scenario_id}/start": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** Create a scenario session and queue workflow execution */
        post: operations["start_scenario_v1_products__product_id__scenarios__scenario_id__start_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/results/{result_artifact_id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Get a frontend-safe normalized workflow result artifact */
        get: operations["get_result_artifact_v1_results__result_artifact_id__get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/scenario-sessions/{scenario_session_id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** Get frontend-safe scenario session state */
        get: operations["get_scenario_session_v1_scenario_sessions__scenario_session_id__get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/v1/scenario-sessions/{scenario_session_id}/next-actions/{next_action_id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** Validate and record a frontend next-action click */
        post: operations["post_next_action_v1_scenario_sessions__scenario_session_id__next_actions__next_action_id__post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
}
export type webhooks = Record<string, never>;
export interface components {
    schemas: {
        /**
         * AtomLabAtomId
         * @enum {string}
         */
        AtomLabAtomId: "A01" | "A02" | "A03" | "A04" | "A05" | "A06" | "A07" | "A08" | "A09" | "A10" | "A11";
        /** AtomLabAtomResponse */
        AtomLabAtomResponse: {
            /** Action Type */
            action_type: string;
            atom_id: components["schemas"]["AtomLabAtomId"];
            /** Base Action Config Id */
            base_action_config_id: string;
            /** Description */
            description: string;
            /** Example Input */
            example_input: {
                [key: string]: unknown;
            };
            /** Input Schema */
            input_schema: {
                [key: string]: unknown;
            };
            /** Output Schema */
            output_schema: {
                [key: string]: unknown;
            };
            /** Prompt */
            prompt: string;
            /** Prompt Ref */
            prompt_ref: string;
            schema_refs: components["schemas"]["AtomLabSchemaRefsResponse"];
        };
        /** AtomLabDebugArtifactResponse */
        AtomLabDebugArtifactResponse: {
            /** Artifact Id */
            artifact_id: string;
            /** Error Code */
            error_code: string | null;
            /** Raw Output Text */
            raw_output_text: string | null;
            /** Redacted */
            redacted: boolean;
            /** Truncated */
            truncated: boolean;
        };
        /** AtomLabErrorDetailResponse */
        AtomLabErrorDetailResponse: {
            /** Code */
            code: string;
            /** Field Errors */
            field_errors: components["schemas"]["AtomLabFieldErrorResponse"][];
            /** Message */
            message: string;
        };
        /** AtomLabErrorResponse */
        AtomLabErrorResponse: {
            error: components["schemas"]["AtomLabErrorDetailResponse"];
            /** Request Id */
            request_id: string;
        };
        /** AtomLabFieldErrorResponse */
        AtomLabFieldErrorResponse: {
            /** Message */
            message: string;
            /** Path */
            path: string;
        };
        /** AtomLabModelRefreshResponse */
        AtomLabModelRefreshResponse: {
            /** Error */
            error: string | null;
            /** Last Success At */
            last_success_at: string | null;
            refresh_status: components["schemas"]["ModelCatalogRefreshStatus"];
            /** Snapshot Id */
            snapshot_id: string | null;
            /** Stale */
            stale: boolean;
        };
        /** AtomLabModelResponse */
        AtomLabModelResponse: {
            /** Allowed Reasoning Efforts */
            allowed_reasoning_efforts: components["schemas"]["ReasoningEffort"][] | null;
            compatibility: components["schemas"]["ModelCatalogCompatibility"];
            /** Model Id */
            model_id: string;
            /** Provenance */
            provenance: {
                [key: string]: {
                    [key: string]: string;
                };
            };
            reason: components["schemas"]["ModelCatalogReason"];
            /** Reasoning Supported */
            reasoning_supported: boolean | null;
        };
        /** AtomLabModelsResponse */
        AtomLabModelsResponse: {
            /** Error */
            error: string | null;
            /** Items */
            items: components["schemas"]["AtomLabModelResponse"][];
            /** Last Success At */
            last_success_at: string | null;
            refresh_status: components["schemas"]["ModelCatalogRefreshStatus"];
            /** Snapshot Id */
            snapshot_id: string | null;
            /** Stale */
            stale: boolean;
        };
        /** AtomLabPresetCreatedResponse */
        AtomLabPresetCreatedResponse: {
            /**
             * Created At
             * Format: date-time
             */
            created_at: string;
            /** Preset Id */
            preset_id: string;
            /** Version */
            version: number;
        };
        /** AtomLabPresetExportResponse */
        AtomLabPresetExportResponse: {
            configuration: components["schemas"]["AtomLabPresetVersionRequest"];
            /**
             * Format Version
             * @default 1
             * @constant
             */
            format_version: 1;
            /** Preset Id */
            preset_id: string;
            /** Version */
            version: number;
        };
        /** AtomLabPresetListResponse */
        AtomLabPresetListResponse: {
            /** Items */
            items: components["schemas"]["AtomLabPresetSummaryResponse"][];
            /** Next Cursor */
            next_cursor: string | null;
        };
        /** AtomLabPresetNextVersionRequest */
        AtomLabPresetNextVersionRequest: {
            atom_id: components["schemas"]["AtomLabAtomId"];
            /** Base Action Config Id */
            base_action_config_id: string;
            /** Base Version */
            base_version: number;
            /** Description */
            description: string;
            /** Example Input */
            example_input: {
                [key: string]: unknown;
            };
            /** Fixed Fields */
            fixed_fields?: string[];
            /** Model Id */
            model_id: string;
            /** Name */
            name: string;
            /** Prompt */
            prompt: string;
            /** Prompt Ref */
            prompt_ref: string;
            reasoning_effort?: components["schemas"]["ReasoningEffort"] | null;
            schema_refs: components["schemas"]["AtomLabSchemaRefsResponse"];
            /** Source Run Id */
            source_run_id?: string | null;
        };
        /** AtomLabPresetSummaryResponse */
        AtomLabPresetSummaryResponse: {
            atom_id: components["schemas"]["AtomLabAtomId"];
            /**
             * Created At
             * Format: date-time
             */
            created_at: string;
            /** Description */
            description: string;
            /** Latest Version */
            latest_version: number;
            /** Name */
            name: string;
            /** Preset Id */
            preset_id: string;
            /**
             * Updated At
             * Format: date-time
             */
            updated_at: string;
        };
        /** AtomLabPresetVersionListResponse */
        AtomLabPresetVersionListResponse: {
            /** Items */
            items: components["schemas"]["AtomLabPresetVersionSummaryResponse"][];
            /** Next Cursor */
            next_cursor: string | null;
        };
        /** AtomLabPresetVersionRequest */
        AtomLabPresetVersionRequest: {
            atom_id: components["schemas"]["AtomLabAtomId"];
            /** Base Action Config Id */
            base_action_config_id: string;
            /** Description */
            description: string;
            /** Example Input */
            example_input: {
                [key: string]: unknown;
            };
            /** Fixed Fields */
            fixed_fields?: string[];
            /** Model Id */
            model_id: string;
            /** Name */
            name: string;
            /** Prompt */
            prompt: string;
            /** Prompt Ref */
            prompt_ref: string;
            reasoning_effort?: components["schemas"]["ReasoningEffort"] | null;
            schema_refs: components["schemas"]["AtomLabSchemaRefsResponse"];
            /** Source Run Id */
            source_run_id?: string | null;
        };
        /** AtomLabPresetVersionResponse */
        AtomLabPresetVersionResponse: {
            atom_id: components["schemas"]["AtomLabAtomId"];
            /** Base Action Config Id */
            base_action_config_id: string;
            /**
             * Created At
             * Format: date-time
             */
            created_at: string;
            /** Description */
            description: string;
            /** Example Input */
            example_input: {
                [key: string]: unknown;
            };
            /** Fixed Fields */
            fixed_fields?: string[];
            /** Model Id */
            model_id: string;
            /** Name */
            name: string;
            /** Preset Id */
            preset_id: string;
            /** Prompt */
            prompt: string;
            /** Prompt Ref */
            prompt_ref: string;
            reasoning_effort?: components["schemas"]["ReasoningEffort"] | null;
            schema_refs: components["schemas"]["AtomLabSchemaRefsResponse"];
            /** Source Run Id */
            source_run_id?: string | null;
            /** Version */
            version: number;
        };
        /** AtomLabPresetVersionSummaryResponse */
        AtomLabPresetVersionSummaryResponse: {
            atom_id: components["schemas"]["AtomLabAtomId"];
            /**
             * Created At
             * Format: date-time
             */
            created_at: string;
            /** Description */
            description: string;
            /** Name */
            name: string;
            /** Preset Id */
            preset_id: string;
            /** Version */
            version: number;
        };
        /** AtomLabProviderCallDiagnosticResponse */
        AtomLabProviderCallDiagnosticResponse: {
            /** Action Run Id */
            action_run_id: string;
            /** Error Code */
            error_code: string | null;
            /** Latency Ms */
            latency_ms: number;
            /** Physical Call Index */
            physical_call_index: number;
            /** Provider Call Id */
            provider_call_id: string;
            /** Response Model Id */
            response_model_id: string | null;
            /** Semantic Attempt Index */
            semantic_attempt_index: number;
            status: components["schemas"]["ProviderCallStatus"];
            /** Transport Attempt Index */
            transport_attempt_index: number;
        };
        /** AtomLabRunAcceptedResponse */
        AtomLabRunAcceptedResponse: {
            /** Job Id */
            job_id: string;
            /** Run Id */
            run_id: string;
            /** Scenario Session Id */
            scenario_session_id: string;
            status: components["schemas"]["AtomLabRunStatus"];
        };
        /** AtomLabRunDetailResponse */
        AtomLabRunDetailResponse: {
            /**
             * Created At
             * Format: date-time
             */
            created_at: string;
            diagnostics: components["schemas"]["AtomLabRunDiagnosticsResponse"];
            /** Finished At */
            finished_at: string | null;
            /** Result */
            result: {
                [key: string]: unknown;
            } | unknown[] | null;
            /** Run Id */
            run_id: string;
            runtime_ids: components["schemas"]["AtomLabRunRuntimeIdsResponse"];
            /** Snapshot */
            snapshot: {
                [key: string]: unknown;
            };
            /** Started At */
            started_at: string | null;
            status: components["schemas"]["AtomLabRunStatus"];
        };
        /** AtomLabRunDiagnosticsResponse */
        AtomLabRunDiagnosticsResponse: {
            /** Debug Artifacts */
            debug_artifacts: components["schemas"]["AtomLabDebugArtifactResponse"][];
            /** Debug Artifacts Truncated */
            debug_artifacts_truncated: boolean;
            /** Duration Ms */
            duration_ms: number | null;
            /** Error Code */
            error_code: string | null;
            /** Physical Calls */
            physical_calls: number;
            /** Provider Calls */
            provider_calls: components["schemas"]["AtomLabProviderCallDiagnosticResponse"][];
            /** Provider Calls Truncated */
            provider_calls_truncated: boolean;
            /** Requested Model Id */
            requested_model_id: string;
            requested_reasoning_effort: components["schemas"]["ReasoningEffort"] | null;
            /** Response Model Id */
            response_model_id: string | null;
            /** Succeeded First Attempt */
            succeeded_first_attempt: boolean | null;
            /** Transport Attempts */
            transport_attempts: number;
            /** Validation Attempts */
            validation_attempts: number;
        };
        /** AtomLabRunListResponse */
        AtomLabRunListResponse: {
            /** Items */
            items: components["schemas"]["AtomLabRunSummaryResponse"][];
            /** Next Cursor */
            next_cursor: string | null;
        };
        /** AtomLabRunRuntimeIdsResponse */
        AtomLabRunRuntimeIdsResponse: {
            /** Action Run Id */
            action_run_id: string | null;
            /** Artifact Id */
            artifact_id: string | null;
            /** Job Id */
            job_id: string | null;
            /** Scenario Session Id */
            scenario_session_id: string | null;
        };
        /**
         * AtomLabRunStatus
         * @enum {string}
         */
        AtomLabRunStatus: "queued" | "running" | "succeeded" | "failed" | "expired" | "cancelled";
        /** AtomLabRunSummaryResponse */
        AtomLabRunSummaryResponse: {
            /** Atom Id */
            atom_id: string;
            /**
             * Created At
             * Format: date-time
             */
            created_at: string;
            /** Finished At */
            finished_at: string | null;
            /** Model Id */
            model_id: string;
            /** Preset Id */
            preset_id: string | null;
            /** Preset Version */
            preset_version: number | null;
            /** Run Id */
            run_id: string;
            /** Started At */
            started_at: string | null;
            status: components["schemas"]["AtomLabRunStatus"];
        };
        /** AtomLabSchemaRefsResponse */
        AtomLabSchemaRefsResponse: {
            input: components["schemas"]["AtomLabVersionedSchemaRefResponse"];
            output: components["schemas"]["AtomLabVersionedSchemaRefResponse"];
        };
        /** AtomLabVersionedSchemaRefResponse */
        AtomLabVersionedSchemaRefResponse: {
            /** Schema Ref */
            schema_ref: string;
            /** Version */
            version: number;
        };
        /** ClientEventRequest */
        ClientEventRequest: {
            /** Event Id */
            event_id: string;
            event_type: components["schemas"]["WebClientEventType"];
            /** Frontend Id */
            frontend_id: string;
            /** Guest Id */
            guest_id?: string | null;
            /** Product Id */
            product_id: string;
            /** Properties */
            properties?: {
                [key: string]: unknown;
            };
            /** Scenario Session Id */
            scenario_session_id?: string | null;
            /** User Id */
            user_id?: string | null;
            /** Web Session Id */
            web_session_id: string;
        };
        /** ClientEventResponse */
        ClientEventResponse: {
            /** Event Id */
            event_id: string;
            event_type: components["schemas"]["WebClientEventType"];
        };
        /** DemoRunRequest */
        DemoRunRequest: {
            /** Demo Id */
            demo_id: string;
            /** Source Text */
            source_text: string;
        };
        /** ErrorDetailResponse */
        ErrorDetailResponse: {
            /** Code */
            code: string;
            /** Message */
            message: string;
            /** Request Id */
            request_id: string;
        };
        /** ErrorResponse */
        ErrorResponse: {
            error: components["schemas"]["ErrorDetailResponse"];
        };
        /**
         * FrontendType
         * @enum {string}
         */
        FrontendType: "chrome_extension" | "web";
        /** GuestIdentityResponse */
        GuestIdentityResponse: {
            /** Guest Id */
            guest_id: string;
        };
        /** HTTPValidationError */
        HTTPValidationError: {
            /** Detail */
            detail?: components["schemas"]["ValidationError"][];
        };
        /** HandoffAcceptRequest */
        HandoffAcceptRequest: {
            /** Guest Id */
            guest_id?: string | null;
            /** Source Frontend Instance Id */
            source_frontend_instance_id?: string | null;
        };
        /** HandoffCreateRequest */
        HandoffCreateRequest: {
            /** Handoff Definition Id */
            handoff_definition_id: string;
            /** Source Artifact Id */
            source_artifact_id: string;
            /** Source Scenario Session Id */
            source_scenario_session_id: string;
        };
        /** HandoffCreateResponse */
        HandoffCreateResponse: {
            /**
             * Expires At
             * Format: date-time
             */
            expires_at: string;
            /** Handoff Id */
            handoff_id: string;
            /** Handoff Token */
            handoff_token: string;
            status: components["schemas"]["HandoffStatus"];
        };
        /** HandoffPreviewResponse */
        HandoffPreviewResponse: {
            /**
             * Expires At
             * Format: date-time
             */
            expires_at: string;
            /** Handoff Id */
            handoff_id: string;
            /** Preview */
            preview: {
                [key: string]: unknown;
            };
            /** Source Product Display Name */
            source_product_display_name: string;
            /** Source Product Id */
            source_product_id: string;
            status: components["schemas"]["HandoffStatus"];
            /** Target Job Id */
            target_job_id?: string | null;
            /** Target Product Display Name */
            target_product_display_name: string;
            /** Target Product Id */
            target_product_id: string;
            /** Target Scenario Id */
            target_scenario_id: string;
            /** Target Scenario Session Id */
            target_scenario_session_id?: string | null;
        };
        /**
         * HandoffStatus
         * @enum {string}
         */
        HandoffStatus: "created" | "viewed" | "accepted" | "declined" | "consumed" | "expired" | "failed";
        /**
         * ModelCatalogCompatibility
         * @enum {string}
         */
        ModelCatalogCompatibility: "compatible" | "unknown" | "unsupported";
        /**
         * ModelCatalogReason
         * @enum {string}
         */
        ModelCatalogReason: "override_compatible" | "override_unknown" | "override_unsupported" | "litellm_metadata_missing" | "litellm_compatibility_incomplete" | "confirmed_openai_text_gpt";
        /**
         * ModelCatalogRefreshStatus
         * @enum {string}
         */
        ModelCatalogRefreshStatus: "current" | "pending" | "running";
        /**
         * ProviderCallStatus
         * @enum {string}
         */
        ProviderCallStatus: "created" | "running" | "succeeded" | "failed" | "timed_out";
        /**
         * QuotaDimension
         * @enum {string}
         */
        QuotaDimension: "product" | "scenario";
        /**
         * QuotaPeriod
         * @enum {string}
         */
        QuotaPeriod: "lifetime";
        /** QuotaStateResponse */
        QuotaStateResponse: {
            /** Dimension Key */
            dimension_key: string;
            /** Exhausted */
            exhausted: boolean;
            /** Guest Id */
            guest_id: string;
            /** Limit Count */
            limit_count: number;
            period: components["schemas"]["QuotaPeriod"];
            /** Product Id */
            product_id: string;
            quota_dimension: components["schemas"]["QuotaDimension"];
            /** Quota Policy Id */
            quota_policy_id: string;
            /** Remaining Count */
            remaining_count: number;
            /** Scenario Id */
            scenario_id?: string | null;
            unit: components["schemas"]["QuotaUnit"];
            /** Used Count */
            used_count: number;
        };
        /**
         * QuotaUnit
         * @enum {string}
         */
        QuotaUnit: "scenario_run";
        /**
         * ReasoningEffort
         * @enum {string}
         */
        ReasoningEffort: "none" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max";
        /** ResultArtifactResponse */
        ResultArtifactResponse: {
            /**
             * Created At
             * Format: date-time
             */
            created_at: string;
            /** Job Id */
            job_id: string;
            /** Output */
            output: {
                [key: string]: unknown;
            };
            /** Result Artifact Id */
            result_artifact_id: string;
            /** Scenario Session Id */
            scenario_session_id: string;
            /** Schema Ref */
            schema_ref: string;
            /** Schema Version */
            schema_version: number;
            /** Workflow Id */
            workflow_id: string;
            /** Workflow Version */
            workflow_version: number;
        };
        /** RuntimeConfigResponse */
        RuntimeConfigResponse: {
            /** Allowed Ui Capabilities */
            allowed_ui_capabilities: string[];
            /** Frontend Ids */
            frontend_ids: string[];
            /** Frontends */
            frontends: components["schemas"]["RuntimeFrontendResponse"][];
            /** Product Id */
            product_id: string;
            quota_summary: components["schemas"]["RuntimeQuotaSummaryResponse"] | null;
            /** Scenario Ids */
            scenario_ids: string[];
            /** Scenarios */
            scenarios: components["schemas"]["RuntimeScenarioResponse"][];
        };
        /** RuntimeFrontendResponse */
        RuntimeFrontendResponse: {
            /** Enabled */
            enabled: boolean;
            /** Frontend Id */
            frontend_id: string;
            type: components["schemas"]["FrontendType"];
        };
        /** RuntimeQuotaSummaryResponse */
        RuntimeQuotaSummaryResponse: {
            dimension: components["schemas"]["QuotaDimension"];
            /** Limit Count */
            limit_count: number;
            period: components["schemas"]["QuotaPeriod"];
            /** Quota Policy Id */
            quota_policy_id: string;
            unit: components["schemas"]["QuotaUnit"];
        };
        /** RuntimeRendererHintResponse */
        RuntimeRendererHintResponse: {
            /**
             * Renderer
             * @default json_schema
             * @constant
             */
            renderer: "json_schema";
            /** Schema Ref */
            schema_ref: string;
            /** Schema Version */
            schema_version?: number | null;
        };
        /** RuntimeScenarioResponse */
        RuntimeScenarioResponse: {
            /** Allowed Next Actions */
            allowed_next_actions?: string[];
            input_renderer_hint: components["schemas"]["RuntimeRendererHintResponse"];
            output_renderer_hint: components["schemas"]["RuntimeRendererHintResponse"];
            /** Scenario Id */
            scenario_id: string;
            /** Version */
            version: number;
        };
        /** ScenarioNextActionRequest */
        ScenarioNextActionRequest: {
            /** Checkpoint Id */
            checkpoint_id: string;
        };
        /** ScenarioSessionResponse */
        ScenarioSessionResponse: {
            /** Allowed Next Actions */
            allowed_next_actions?: string[];
            /** Current Checkpoint Id */
            current_checkpoint_id?: string | null;
            /** Job Id */
            job_id: string | null;
            /** Result Artifact Id */
            result_artifact_id?: string | null;
            /** Scenario Session Id */
            scenario_session_id: string;
            status: components["schemas"]["ScenarioSessionStatus"];
        };
        /**
         * ScenarioSessionStatus
         * @enum {string}
         */
        ScenarioSessionStatus: "started" | "waiting_for_user" | "running" | "completed" | "failed" | "expired";
        /** ScenarioStartRequest */
        ScenarioStartRequest: {
            /** Frontend Id */
            frontend_id: string;
            /** Guest Id */
            guest_id?: string | null;
            /** Input */
            input: unknown;
            /** Source Frontend Instance Id */
            source_frontend_instance_id?: string | null;
            /** User Id */
            user_id?: string | null;
        };
        /** ScenarioStartResponse */
        ScenarioStartResponse: {
            /** Allowed Next Actions */
            allowed_next_actions?: string[];
            /** Job Id */
            job_id: string;
            /** Result Artifact Id */
            result_artifact_id?: string | null;
            /** Scenario Session Id */
            scenario_session_id: string;
            status: components["schemas"]["ScenarioSessionStatus"];
        };
        /** ValidationError */
        ValidationError: {
            /** Context */
            ctx?: Record<string, never>;
            /** Input */
            input?: unknown;
            /** Location */
            loc: (string | number)[];
            /** Message */
            msg: string;
            /** Error Type */
            type: string;
        };
        /**
         * WebClientEventType
         * @description ANY-17 v1 web-event allowlist -- the only event types POST /v1/client-events accepts.
         *
         *     A closed HTTP-facing enum per docs/agent/coding-conventions.md, not a plain `str`, so OpenAPI
         *     emits a real enum and `ce-kit` can generate its client-side union from it instead of keeping an
         *     independent hand-maintained copy.
         * @enum {string}
         */
        WebClientEventType: "web.product_viewed" | "web.form_started" | "web.form_submitted" | "web.result_viewed" | "web.retry_clicked" | "web.mode_selected" | "web.gap_selected" | "web.feedback_submitted";
    };
    responses: never;
    parameters: never;
    requestBodies: never;
    headers: never;
    pathItems: never;
}
export type $defs = Record<string, never>;
export interface operations {
    health_health_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": {
                        [key: string]: string;
                    };
                };
            };
        };
    };
    list_atoms_v1_atom_lab_atoms_get: {
        parameters: {
            query?: never;
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabAtomResponse"][];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    get_atom_v1_atom_lab_atoms__atom_id__get: {
        parameters: {
            query?: never;
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path: {
                atom_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabAtomResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom not found. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    list_models_v1_atom_lab_models_get: {
        parameters: {
            query?: never;
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabModelsResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    request_model_refresh_v1_atom_lab_models_refresh_post: {
        parameters: {
            query?: never;
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            202: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabModelRefreshResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    list_presets_v1_atom_lab_presets_get: {
        parameters: {
            query?: {
                limit?: number;
                cursor?: string | null;
            };
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabPresetListResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    create_preset_v1_atom_lab_presets_post: {
        parameters: {
            query?: never;
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["AtomLabPresetVersionRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            201: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabPresetCreatedResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Conflict */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    list_preset_versions_v1_atom_lab_presets__preset_id__versions_get: {
        parameters: {
            query?: {
                limit?: number;
                cursor?: string | null;
            };
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path: {
                preset_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabPresetVersionListResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    create_preset_version_v1_atom_lab_presets__preset_id__versions_post: {
        parameters: {
            query?: never;
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path: {
                preset_id: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["AtomLabPresetNextVersionRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            201: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabPresetCreatedResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Conflict */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    get_preset_version_v1_atom_lab_presets__preset_id__versions__version__get: {
        parameters: {
            query?: never;
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path: {
                preset_id: string;
                version: number;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabPresetVersionResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    export_preset_version_v1_atom_lab_presets__preset_id__versions__version__export_get: {
        parameters: {
            query?: never;
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path: {
                preset_id: string;
                version: number;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabPresetExportResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    list_runs_v1_atom_lab_runs_get: {
        parameters: {
            query?: {
                limit?: number;
                cursor?: string | null;
            };
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabRunListResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    start_run_v1_atom_lab_runs_post: {
        parameters: {
            query?: never;
            header: {
                "Idempotency-Key": string;
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": {
                    /**
                     * AtomLabAtomId
                     * @enum {string}
                     */
                    atom_id: "A01" | "A02" | "A03" | "A04" | "A05" | "A06" | "A07" | "A08" | "A09" | "A10" | "A11";
                    /** Input */
                    input: {
                        [key: string]: unknown;
                    };
                    /** Model Id */
                    model_id: string;
                    preset_ref?: {
                        /** Preset Id */
                        preset_id: string;
                        /** Version */
                        version: number;
                    } | null;
                    /** Prompt */
                    prompt: string;
                    reasoning_effort: ("none" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max") | null;
                };
            };
        };
        responses: {
            /** @description Successful Response */
            202: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabRunAcceptedResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Conflict */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab payload is too large. */
            413: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Too Many Requests */
            429: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    get_run_v1_atom_lab_runs__run_id__get: {
        parameters: {
            query?: never;
            header?: {
                "X-Atom-Lab-Access-Code"?: string | null;
            };
            path: {
                run_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabRunDetailResponse"];
                };
            };
            /** @description Atom Lab access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab request validation failed. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
            /** @description Atom Lab is not configured. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["AtomLabErrorResponse"];
                };
            };
        };
    };
    post_client_event_v1_client_events_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["ClientEventRequest"];
            };
        };
        responses: {
            /** @description Event was durably recorded (or already existed for this event_id -- duplicate delivery is idempotent). */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "event_id": "web_evt_123",
                     *       "event_type": "web.product_viewed"
                     *     }
                     */
                    "application/json": components["schemas"]["ClientEventResponse"];
                };
            };
            /** @description Safe response when the supplied guest identity or scenario session is unknown. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "error": {
                     *         "code": "guest_identity_not_found",
                     *         "message": "Guest identity not found.",
                     *         "request_id": "req_123"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description The supplied event_id was already used to record a client event with different content (event type, correlation, or properties). */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "error": {
                     *         "code": "client_event_id_conflict",
                     *         "message": "event_id was already used to record a different client event.",
                     *         "request_id": "req_123"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Safe response for an unknown event type, an invalid product/frontend combination, a disallowed property, or a missing/oversized identifier. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "error": {
                     *         "code": "client_event_type_not_allowed",
                     *         "message": "Event type is not part of the v1 client-event allowlist.",
                     *         "request_id": "req_123"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
        };
    };
    start_demo_run_v1_demo_runs_post: {
        parameters: {
            query?: never;
            header?: {
                "X-Demo-Access-Code"?: string | null;
            };
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["DemoRunRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ScenarioStartResponse"];
                };
            };
            /** @description Demo access denied. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Another demo run is active. */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Demo input is invalid. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Daily demo limit exhausted. */
            429: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Demo runtime is unavailable. */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
        };
    };
    create_handoff_v1_handoffs_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["HandoffCreateRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HandoffCreateResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Conflict */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    get_handoff_v1_handoffs__handoff_token__get: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                handoff_token: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HandoffPreviewResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    accept_handoff_v1_handoffs__handoff_token__accept_post: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                handoff_token: string;
            };
            cookie?: never;
        };
        requestBody?: {
            content: {
                "application/json": components["schemas"]["HandoffAcceptRequest"] | null;
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HandoffPreviewResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Conflict */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Gone */
            410: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
            /** @description Too Many Requests */
            429: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
        };
    };
    decline_handoff_v1_handoffs__handoff_token__decline_post: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                handoff_token: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HandoffPreviewResponse"];
                };
            };
            /** @description Not Found */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Conflict */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Gone */
            410: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    create_guest_identity_v1_identity_guest_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Frontend-safe opaque guest identifier. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "guest_id": "guest_123"
                     *     }
                     */
                    "application/json": components["schemas"]["GuestIdentityResponse"];
                };
            };
        };
    };
    get_product_quota_v1_products__product_id__quota_get: {
        parameters: {
            query: {
                guest_id: string;
                scenario_id?: string | null;
            };
            header?: never;
            path: {
                product_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Current backend-owned guest quota state. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "dimension_key": "kernel_demo",
                     *       "exhausted": false,
                     *       "guest_id": "guest_123",
                     *       "limit_count": 3,
                     *       "period": "lifetime",
                     *       "product_id": "kernel_demo",
                     *       "quota_dimension": "product",
                     *       "quota_policy_id": "kernel_demo.guest_quota_v1",
                     *       "remaining_count": 2,
                     *       "unit": "scenario_run",
                     *       "used_count": 1
                     *     }
                     */
                    "application/json": components["schemas"]["QuotaStateResponse"];
                };
            };
            /** @description Safe response when product or guest identity is unknown. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    get_runtime_config_v1_products__product_id__runtime_config_get: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                product_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Frontend-safe runtime metadata for a configured product. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "allowed_ui_capabilities": [
                     *         "capture_email",
                     *         "continue_to_target",
                     *         "copy_result",
                     *         "create_handoff",
                     *         "render_input",
                     *         "render_output",
                     *         "view_paywall"
                     *       ],
                     *       "frontend_ids": [
                     *         "kernel_demo_ce",
                     *         "web_mirror"
                     *       ],
                     *       "frontends": [
                     *         {
                     *           "enabled": true,
                     *           "frontend_id": "kernel_demo_ce",
                     *           "type": "chrome_extension"
                     *         },
                     *         {
                     *           "enabled": true,
                     *           "frontend_id": "web_mirror",
                     *           "type": "web"
                     *         }
                     *       ],
                     *       "product_id": "kernel_demo",
                     *       "quota_summary": {
                     *         "dimension": "product",
                     *         "limit_count": 3,
                     *         "period": "lifetime",
                     *         "quota_policy_id": "kernel_demo.guest_quota_v1",
                     *         "unit": "scenario_run"
                     *       },
                     *       "scenario_ids": [
                     *         "kernel_demo.single_action_smoke_v1",
                     *         "kernel_demo.multi_step_workflow_smoke_v1",
                     *         "kernel_demo.detect_questions_smoke_v1",
                     *         "kernel_demo.quota_exhausted_smoke_v1",
                     *         "kernel_demo.handoff_smoke_source_v1",
                     *         "kernel_demo.handoff_smoke_target_v1"
                     *       ],
                     *       "scenarios": [
                     *         {
                     *           "allowed_next_actions": [
                     *             "copy_result",
                     *             "create_handoff"
                     *           ],
                     *           "input_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.generic_text_input_v1",
                     *             "schema_version": 1
                     *           },
                     *           "output_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.extract_output_v1",
                     *             "schema_version": 1
                     *           },
                     *           "scenario_id": "kernel_demo.single_action_smoke_v1",
                     *           "version": 1
                     *         },
                     *         {
                     *           "allowed_next_actions": [
                     *             "copy_result",
                     *             "create_handoff"
                     *           ],
                     *           "input_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.generic_text_input_v1",
                     *             "schema_version": 1
                     *           },
                     *           "output_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel.schemas.generate_document_output_v1",
                     *             "schema_version": 1
                     *           },
                     *           "scenario_id": "kernel_demo.multi_step_workflow_smoke_v1",
                     *           "version": 1
                     *         },
                     *         {
                     *           "allowed_next_actions": [
                     *             "copy_result",
                     *             "create_handoff"
                     *           ],
                     *           "input_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.generic_text_input_v1",
                     *             "schema_version": 1
                     *           },
                     *           "output_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel.schemas.generate_questions_output_v1",
                     *             "schema_version": 1
                     *           },
                     *           "scenario_id": "kernel_demo.detect_questions_smoke_v1",
                     *           "version": 1
                     *         },
                     *         {
                     *           "allowed_next_actions": [
                     *             "capture_email",
                     *             "view_paywall"
                     *           ],
                     *           "input_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.generic_text_input_v1",
                     *             "schema_version": 1
                     *           },
                     *           "output_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.extract_output_v1",
                     *             "schema_version": 1
                     *           },
                     *           "scenario_id": "kernel_demo.quota_exhausted_smoke_v1",
                     *           "version": 1
                     *         },
                     *         {
                     *           "allowed_next_actions": [
                     *             "continue_to_target"
                     *           ],
                     *           "input_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.generic_text_input_v1",
                     *             "schema_version": 1
                     *           },
                     *           "output_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.extract_output_v1",
                     *             "schema_version": 1
                     *           },
                     *           "scenario_id": "kernel_demo.handoff_smoke_source_v1",
                     *           "version": 1
                     *         },
                     *         {
                     *           "allowed_next_actions": [
                     *             "copy_result"
                     *           ],
                     *           "input_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.generic_text_input_v1",
                     *             "schema_version": 1
                     *           },
                     *           "output_renderer_hint": {
                     *             "renderer": "json_schema",
                     *             "schema_ref": "kernel_demo.extract_output_v1",
                     *             "schema_version": 1
                     *           },
                     *           "scenario_id": "kernel_demo.handoff_smoke_target_v1",
                     *           "version": 1
                     *         }
                     *       ]
                     *     }
                     */
                    "application/json": components["schemas"]["RuntimeConfigResponse"];
                };
            };
            /** @description Safe not-found response when the product is unknown. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "error": {
                     *         "code": "product_not_found",
                     *         "message": "Product not found",
                     *         "request_id": "req_123"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    start_scenario_v1_products__product_id__scenarios__scenario_id__start_post: {
        parameters: {
            query?: never;
            header?: {
                "Idempotency-Key"?: string | null;
                "X-Live-Canary-Token"?: string | null;
            };
            path: {
                scenario_id: string;
                product_id: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["ScenarioStartRequest"];
            };
        };
        responses: {
            /** @description Stable queue-and-return response for CE polling. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "allowed_next_actions": [],
                     *       "job_id": "job_123",
                     *       "scenario_session_id": "scenario_session_123",
                     *       "status": "started"
                     *     }
                     */
                    "application/json": components["schemas"]["ScenarioStartResponse"];
                };
            };
            /** @description Safe response when the scenario is unknown for the product or the supplied guest identity is unknown. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Idempotency-Key was reused with a request whose scope or input does not match the original request. No new scenario session is created and quota is not consumed; retry with a new Idempotency-Key or the original request. */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "error": {
                     *         "code": "idempotency_key_conflict",
                     *         "message": "Idempotency-Key was already used with a different request.",
                     *         "request_id": "req_123"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Safe validation response for unsupported frontend, invalid input, missing guest identity, or an oversized Idempotency-Key. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Quota exhausted. The backend rejects the scenario start before creating any scenario session or linked job. */
            429: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "error": {
                     *         "code": "quota_exhausted",
                     *         "message": "Guest quota exhausted.",
                     *         "request_id": "req_123"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
        };
    };
    get_result_artifact_v1_results__result_artifact_id__get: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                result_artifact_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Frontend-safe normalized canonical workflow result. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "created_at": "2026-01-01T00:00:00Z",
                     *       "job_id": "job_123",
                     *       "output": {
                     *         "missing_fields": [
                     *           "two"
                     *         ],
                     *         "values": {
                     *           "one": "example value"
                     *         }
                     *       },
                     *       "result_artifact_id": "artifact_123",
                     *       "scenario_session_id": "scenario_session_123",
                     *       "schema_ref": "kernel_demo.extract_output_v1",
                     *       "schema_version": 1,
                     *       "workflow_id": "kernel_demo.single_action_extract_v1",
                     *       "workflow_version": 1
                     *     }
                     */
                    "application/json": components["schemas"]["ResultArtifactResponse"];
                };
            };
            /** @description Safe response when the artifact is unknown, out of tenant/region scope, or is not an available canonical frontend-safe result (raw/debug artifact, unfinished job, or a schema/version mismatch). */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    get_scenario_session_v1_scenario_sessions__scenario_session_id__get: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                scenario_session_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Frontend-safe session snapshot for polling. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "allowed_next_actions": [
                     *         "copy_result",
                     *         "create_handoff"
                     *       ],
                     *       "current_checkpoint_id": "result_ready",
                     *       "job_id": "job_123",
                     *       "result_artifact_id": "artifact_123",
                     *       "scenario_session_id": "scenario_session_123",
                     *       "status": "completed"
                     *     }
                     */
                    "application/json": components["schemas"]["ScenarioSessionResponse"];
                };
            };
            /** @description Safe response when the scenario session is unknown. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "error": {
                     *         "code": "scenario_session_not_found",
                     *         "message": "Scenario session not found.",
                     *         "request_id": "req_123"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    post_next_action_v1_scenario_sessions__scenario_session_id__next_actions__next_action_id__post: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                scenario_session_id: string;
                next_action_id: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["ScenarioNextActionRequest"];
            };
        };
        responses: {
            /** @description Validated next action with the current safe session snapshot. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "allowed_next_actions": [
                     *         "copy_result",
                     *         "create_handoff"
                     *       ],
                     *       "current_checkpoint_id": "result_ready",
                     *       "job_id": "job_123",
                     *       "result_artifact_id": "artifact_123",
                     *       "scenario_session_id": "scenario_session_123",
                     *       "status": "completed"
                     *     }
                     */
                    "application/json": components["schemas"]["ScenarioSessionResponse"];
                };
            };
            /** @description Safe response when the scenario session is unknown. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "error": {
                     *         "code": "scenario_session_not_found",
                     *         "message": "Scenario session not found.",
                     *         "request_id": "req_123"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Safe response for stale checkpoint or disallowed action. */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    /**
                     * @example {
                     *       "error": {
                     *         "code": "scenario_checkpoint_conflict",
                     *         "message": "Scenario checkpoint no longer matches the requested action.",
                     *         "request_id": "req_123"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["ErrorResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
}
