...
def _table_exists(table_name: str) -> bool:
    return not context.is_offline_mode() and sa.inspect(op.get_bind()).has_table(
        table_name,
...
def _named_object_exists(
    ...
    objects = getattr(inspector, inspector_method)(table_name, schema=PLATFORM_SCHEMA)
    return object_name in {item["name"] for item in objects}


def upgrade() -> None:
    presets_existed = _table_exists("atom_lab_presets")
    if not presets_existed:
        op.create_table(
            "atom_lab_presets",
    ...
    if not _named_object_exists("atom_lab_presets", "ix_atom_lab_presets_scope", "get_indexes"):
        op.create_index(
    ...
    if not _named_object_exists(
        "atom_lab_presets", "ix_atom_lab_presets_created_at", "get_indexes"
    ):
        op.create_index(
    ...
    versions_existed = _table_exists("atom_lab_preset_versions")
    if not versions_existed:
        ...
        if not _named_object_exists(
            "atom_lab_preset_versions",
            "fk_atom_lab_preset_versions_preset",
            "get_foreign_keys",
        ):
        ...
        if not _named_object_exists(
            "atom_lab_preset_versions",
            "fk_atom_lab_preset_versions_source_run",
            "get_foreign_keys",
        ):
            op.create_foreign_key(
                "fk_atom_lab_preset_versions_source_run",
        ...
        if not _named_object_exists(
            "atom_lab_preset_versions",
            "uq_atom_lab_preset_versions_identity",
