...
- (2026-08-25, code review finding) `run()`'s `exit_code` was derived purely from per-case
  pass/fail counts; both cost-safety abort branches (`LIVE001`/`LIVE012`) mark every case still
  in `remaining` as failed, but if the tripping case is the *last* one in the queue, `remaining`
  is already empty -- nothing gets marked failed, every case (including the one that tripped the
  cap) reports `status: "pass"`, and the run silently exits `0` despite violating its own cost
  cap. New `cost_safety_aborted: bool` flag, set in both abort branches, now participates directly
  in `exit_code` (`... and not cost_safety_aborted`) -- independent of whether any case happened
  to need marking. The tripping case itself keeps its own real `status: "pass"` (its ledger/schema
  correctness is a separate question from the cost cap), matching `write_evidence_report()`'s own
  already-allowed shape (`exit_code=1` with every individual `case.status == "pass"` -- the
  PROOF008/PROOF009 empty-case-guard pattern).
- (2026-08-25, code review finding) `write_evidence_report()` serialized `asdict(case)` straight
  through `json.dumps()` with Python's default (lax) float handling -- a `math.inf`
  `estimated_cost` (the new fail-closed value from the two findings above) serializes as the bare
  token `Infinity`, which is not valid JSON per RFC 8259 and would fail to parse in a strict or
  non-Python consumer of this evidence report. New `_json_safe()` recursively replaces any
  non-finite float with `None` (the same meaning `None` already carries for every other nullable
  field in this report) before the payload is handed to `json.dumps()`.
...
- [x] (2026-08-25) Fixed an eighth human code review's 1 P1 finding: the `_safe_step_cost()`
      ...
      check` 928 (was 926).
- [x] (2026-08-25) Fixed a ninth human code review's 1 P1 + 1 P2 finding: (P1) `run()`'s
      `exit_code` silently read `0` when the cost cap tripped on the very last queued case
      (`remaining` empty, nothing left to mark failed) -- new `cost_safety_aborted` flag now
      participates in `exit_code` directly, independent of `remaining`. (P2) `math.inf`
      `estimated_cost` serialized as the invalid-JSON `Infinity` token -- new `_json_safe()`
      normalizes any non-finite float to `None` before `json.dumps()` (also fixed a bug in
      `_json_safe()` itself while writing its regression test: `asdict()` preserves `EvidenceCase.
      steps`'s original `tuple` type, which the first draft's `isinstance(value, list)` check
      didn't recurse into, so the very `Infinity` this function exists to strip was passing
      through unnoticed -- added `tuple` alongside `list`, caught by the new test immediately). 2
      new regression tests (cap-trips-on-last-case in `tests/test_live_canary.py`,
      `Infinity`-in-serialized-output in `tests/test_atoms_proof.py`). `quick-check` 930 (was
      928). Did not re-run the credentialed live-provider cycle again -- both fixes are no-ops for
      real OpenAI's always-positive-finite, always-under-cap costs (see Decision log).
...
| 2026-08-25 | `cost_safety_aborted` is a plain local `bool` in `run()`, not a new field on `EvidenceCase`/the evidence-report payload | `exit_code` is the only thing that needed fixing (the observable bug was "the process exits 0"); the *evidence JSON* already correctly records every skipped case as `LIVE001`/`LIVE012` `fail` whenever `remaining` was non-empty, and the one case that actually triggered the abort already carries its own real `status`/`error_code`/`error_message` -- there was never a missing fact in the report itself, only a wrong exit code derived from it. Adding a redundant top-level flag to the payload would duplicate information already inferable from `cases` (or, worse, invite a second place that could drift out of sync with it). |
| 2026-08-25 | `_json_safe()` normalizes to `None` at serialization time (`write_evidence_report()`), not by changing `_safe_raw_cost()`/`_safe_step_cost()` to return `None` instead of `math.inf` in the first place | `run()`'s cost-cap comparison (`total_cost > max_total_cost_usd`) needs an actual number that's guaranteed to exceed any finite cap -- `None` can't be compared with `>` to a float without special-casing every comparison site, while `math.inf` reuses Python's normal numeric comparison semantics for free. Keeping the poison value as `math.inf` in-memory and only sanitizing it at the JSON boundary keeps the cap-tripping logic simple and the two concerns (fail-closed arithmetic vs. valid-JSON output) separable. |
| 2026-08-25 | Did not re-run the credentialed live-provider cycle after this fix either | Same reasoning as the `_safe_raw_cost()` row above, for both findings this round: the `exit_code` fix only changes behavior when the cost cap actually trips, which a real run at ~$0.008 total (against a $0.50 default cap) never approaches; the JSON-safety fix only changes behavior when a non-finite cost value already exists, which real provider data never produces. Neither fix has any effect on what the existing evidence demonstrates. |
...
| 2026-08-25 | Fixed a ninth human code review's 1 P1 + 1 P2 (HEAD `ec852a7b`, verdict REQUEST_CHANGES; confirmed the prior round's netting-out P1 was closed correctly). P1: `run()`'s `exit_code` silently read `0` when the cost cap tripped on the very last queued case, since the "mark `remaining` as failed" loop has nothing to do once `remaining` is empty -- new `cost_safety_aborted` flag now participates in `exit_code` directly. P2: `math.inf` `estimated_cost` serialized to the RFC-8259-invalid `Infinity` JSON token -- new `_json_safe()` normalizes any non-finite float to `None` in the payload before `json.dumps()`. Wrote the exact last-case regression test the review asked for, plus an `Infinity`-in-output test -- the latter caught a real bug in `_json_safe()` itself on the first attempt (`asdict()` preserves `EvidenceCase.steps`'s original `tuple` type; the first draft's `isinstance(value, list)` check silently skipped it, so `Infinity` was still leaking through), fixed immediately by also checking `tuple`. `quick-check` 930 (was 928). Confirmed the review's other items were already stale: CI is fully green on the prior HEAD (`gh pr checks 84`), and the PR body's `quick-check`/evidence wording was already synced last round. Did not re-run the credentialed cycle -- both fixes are no-ops for real provider data (see Decision log). | Push, await the standing human review from `gushinets`. |
