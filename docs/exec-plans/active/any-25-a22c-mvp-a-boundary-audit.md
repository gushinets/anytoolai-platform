...
- Next action: none — implementation, three internal code-review rounds, and twenty-three GitHub
  ...
  ... }` body) feeding the same timeline/scope machinery; move to `completed/` once merged.
- Blocker: none
...
- Known, documented ceilings of the shared resolver (`ponytail:` comments in
  ...
  scope (narrower — can only miss an in-scope `var`, never resolve one that isn't), a reassignment
  inside a conditional branch (`if (cond) { role = "x"; }`) attaches to its enclosing binding's
  write timeline unconditionally, as if that branch always executes (biases toward *more*
  detections, never fewer), a reassignment reachable from a declaration's own scope can attach to
  the wrong sibling shadowing declaration of the same name (over-attaches, doesn't under-detect),
  a concise-body arrow parameter default (`(role = "x") => role`, no `{ ... }` block to anchor
  the parameter's scope to) isn't modeled, only bare `${NAME}` template holes fold, only relative
  named imports propagate, and a bare Python `import a.b` (no `as`) isn't followed. Upgrade path
  for all of them is a real JS/TS parser; none is a bypass a reviewer has demonstrated. (Round
  34's "one flat namespace, no scope tracking" ceiling was closed in round 36; round 36's own
  "any reassignment invalidates the binding" ceiling was closed in round 37 — the resolver now
  tracks a write timeline and resolves the value actually in effect at each use site.)
