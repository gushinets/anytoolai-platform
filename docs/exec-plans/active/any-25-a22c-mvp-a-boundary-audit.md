...
- Next action: none — implementation, three internal code-review rounds, and twenty-three GitHub
  ...
  matched no bare package name in `PROVIDER_JS_PACKAGES`); only the temp-tree-regression-fixture
  ask remains deferred as follow-up debt — see those entries); move to `completed/` once merged.
- Blocker: none
