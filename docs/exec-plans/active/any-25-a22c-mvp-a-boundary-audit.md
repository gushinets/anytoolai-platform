...
- Owner: agent
- Created: 2026-08-31
- Last updated: 2026-09-03 (round 70)
...
- Next action: none — implementation, three internal code-review rounds, and twenty-three GitHub
  ...
  *expression* evaluation — a write behind `&&`/`||`/`??`'s short-circuited right side or a
  ...
  elements, computed keys, ternary/short-circuit values). Round 64 found `defaultedValues`'s own
  absence check was the one place in the whole path model still asking a *different* state
  model's question — "does the source's original literal declare this key," blind to the same
  member-write timeline everything else now goes through — fixed with `definitelyHasPath`, a
  presence-side counterpart to the value replay that answers the identical question ("is this key
  here") off the identical timeline, paired with the existing structural container check so a
  genuinely opaque source still never gets a guessed default. Round 65 found that "presence" itself
  was still the wrong question — a real destructuring default fires on `undefined`, not on
  absence — and that the round-64 replay only ever accumulated `false → true` instead of replacing
  on a deterministic write, so it couldn't un-prove presence either; fixed by asking "is this
  guaranteed *non-`undefined`*" (an explicit `= undefined` and `void x` disqualify; everything else
  self-contained counts) with real replace/AND semantics matching the value replay's own. Round 66
  found that "everything else self-contained counts" was itself too generous — a `CallExpression`
  is self-contained too, and can plainly return `undefined` — narrowed the leaf check to an
  explicit allowlist of structurally-safe shapes (string/template/numeric literals, `+`), with
  `??`/`||` given their own operator-specific rule (the fallback alone can guarantee the whole
  expression) that `&&` doesn't share. Round 67 found that allowlist itself was too narrow in the
  *other* direction — a destructuring default fires only on `undefined`, never on `null` or any
  other falsy-but-defined value, so `null`/`false`/other primitive literals falling through to
  "not guaranteed" wrongly injected a default that could never actually run; extended the
  allowlist to every atomic JS/TS primitive-literal token (`null`, `true`, `false`, BigInt, regex).
  Round 68 ("Code-review (team lead #4)", re-reviewing commit `3ffb25b5`) reported three fresh
  boundary bypasses, none in the JS/TS resolver this round: the prompt gate's key-scanning regexes
  only ever matched a `role:`/`instructions:` object-literal key or ES2015 shorthand, so a staged
  request object built via member assignment (`req.role = SYSTEM_ROLE;`,
  `request["role"] = PROMPT;`) hit neither shape even though the shared JS resolver could already
  resolve the RHS — fixed with two more regexes (`_JS_DOT_ASSIGN_KEY_RE`/
  `_JS_BRACKET_ASSIGN_KEY_RE`) reusing the same `js_string_expr_at` lookup the colon form already
  used, no `.mjs` resolver change needed. The other two were both in the shared Python resolver
  (`static_string_resolution.py`), previously-documented, deliberately-deferred debt that this
  round finally closed: `_add_module_level_constants` walked `tree.body` directly, so a module-
  level constant reassigned inside an `if` (`PATH = "/safe"; if enabled: PATH =
  "/proposal_ai/status"`) was invisible — fixed at the model level, not the one call site named,
  since `PythonModules.constants` moved from a single `str` per name to a `frozenset[str]` of
  every reachable value (mirroring the JS side's own existing per-position model, which the
  module's own docstring already promised for both languages), with a deterministic write
  replacing the set and a write inside an `ast.If` branch (recursed into, covering `elif`/nested
  `if`) unioning into it instead; `python_string_value` (renamed `python_expr_values`) and its
  three call sites, plus `python_string_values`'s tree-walker (shared by the LiteLLM and provider-
  host gates too, so the same fix closes the identical bypass class there for free), moved to the
  set-returning shape together, deliberately keeping `+`/f-string folding restricted to single-
  valued operands rather than a Cartesian product nothing here needs yet. And
  `module_import_aliases`'s own docstring named its scope cut precisely: bare `import pkg.paths`
  (no `as`) binds only the top-level name `pkg`, so `pkg.paths.PROPOSAL_STATUS_PATH` — an ordinary
  two-level attribute chain — couldn't structurally match the old one-hop-only `ast.Attribute`
  case; generalized (not hardcoded to two levels) via a new `bare_import_roots` set plus an
  `_attribute_chain` walk that resolves any depth of dotted chain rooted at a bare-imported name
  against the existing `module_paths` suffix index. All three findings were reproduced against
  current code first, via two parallel Explore agents, before any fix was designed. New
  regressions: 4 in the JS prompt gate (dot-assignment `role`, bracket-assignment `role`, dot-
  assignment `instructions`, and a `===`-comparison negative control proving `(?!=)` rejects it)
  and 3 in the Python endpoint gate (the conditional-reassignment case, a safe-branches-only
  negative control, and the bare-dotted-import case). `pytest tests/architecture` — 195 passed (up
  from 188); `validate_architecture.py` and `validate-docs` — both clean; `quick-check` (clean
  `scripts/agent/node_modules`) — 1152 passed (up from 1145), all green. Round 69 ("Code-review
  (me #53)", reviewing commit `0ed90671`) found round 68's own new Python conditional-value model
  had two follow-on gaps in exactly the way round 68's own Follow-up debt entry had flagged as a
  deliberate cut: `python_expr_values`'s `+`/f-string folding required every operand/part to
  resolve to exactly one value, so composing a conditionally-reachable name with a literal
  (`PREFIX + "/status"` or `f"{PREFIX}/status"`) went straight back to unresolved instead of
  folding to the Cartesian product of both — reproduced against current code first (`offenders ==
  []` for both the `+` and f-string forms, confirmed the bypass). Fixed with `_combine`, a direct
  port of `js_scope_resolver.mjs`'s own `combine()` (same `_MAX_COMBINATIONS = 256` cap, same
  reasoning), used by both `BinOp(+)` and `JoinedStr` folding. Separately, the review also found a
  genuine branch-state correctness bug self-introduced by round 68's own conditional model: each
  `ast.If` branch had been unioning its write straight into the single live, already-accumulated
  state, so a value written *before* the `if` stayed reachable forever even once every runtime path
  through the `if`/`else` unconditionally overwrote it (`PATH = "/proposal_ai/status"; if flag:
  PATH = "/safe-a" else: PATH = "/safe-b"` kept the forbidden initial value falsely reachable) —
  reproduced first (confirmed a false-positive offender on that exact case). Fixed by evaluating
  each branch from an independent copy of the *entry* state and joining only the two branches'
  *resulting* states by union at the end, rather than mutating one running state as writes are
  found — the join, not the individual write, is now the union point. New regressions: 4 in the
  Python endpoint gate (conditional constant composed via `+`, the same via f-string, both branches
  overwriting a forbidden value drops it at the join, and sequential writes inside one branch leave
  only the final one reaching the join). `pytest tests/architecture` — 199 passed (up from 195);
  `validate_architecture.py` and `validate-docs` — both clean; `quick-check` (clean
  `scripts/agent/node_modules`) — 1156 passed (up from 1152), all green. Round 70 ("Code-review
  (me #54)", reviewing commit `d1874378`) confirmed both round 69 fixes hold, and found
  `_add_module_level_constants` still only ever matched `ast.Assign`/`ast.AnnAssign` — `ast.AugAssign`
  (`PATH += "x"`) fell through the `else: continue` untouched, so a fully deterministic, statically-
  known route path assembled with `+=` (`PATH = "/proposal"; PATH += "_ai/status"`) left the
  resolver holding only the stale pre-assignment value — reproduced against current code first
  (`offenders == []`). Fixed in the shared resolver, not the endpoint gate, per the review's own
  framing: `ast.AugAssign` with `ast.Add` on an `ast.Name` target is now handled like any other
  deterministic write, replacing the name's reachable set with `_combine([previous, rhs])` (the
  same Cartesian-product helper round 69 introduced) — so it composes correctly with a name that
  already has more than one reachable value, and inherits round 69's entry-state/join machinery for
  free when it appears inside an `ast.If` branch, with no extra branch-state code needed. New
  regressions: the reviewer's own deterministic `+=` case, and a conditional `+=` reachable on only
  one branch, checking both the original and the augmented result survive to the join. `pytest
  tests/architecture` — 201 passed (up from 199); `validate_architecture.py` and `validate-docs` —
  both clean; `quick-check` (clean `scripts/agent/node_modules`) — 1158 passed (up from 1156), all
  green; move to `completed/` once merged.
- Blocker: none
...
| 2026-09-03 | Round 64 — review ("Code-review (me #49)", reviewing commit `30634133`) confirmed round 63's nested-destructuring path model holds, both for replacement-before-snapshot and replacement-after-snapshot, and found one remaining place still asking the *old* question: `defaultedValues`'s absence check (`literalLacks` on an `owners` container fetched at an empty/shallow path) only ever saw the source's *original* literal shape, since a member write's chain (depth ≥ 1) is invisible to a `replayPath` query whose own `path` is shorter than that depth — so `config.provider = "internal";` before `const { provider = "openai" } = config;` never stopped `"openai"` from being wrongly treated as reachable, a false positive that (per the review) can fail architecture validation on completely safe code. Reproduced against current code first (three cases: a deterministic property added to an empty object before the destructuring, the same but conditional, and the equivalent `preset`/`role` prompt case) — all three showed the bug. Root-caused per the standing rule from this same conversation (fix the model, not the instance): the value question ("what is `config.provider`") and the absence question ("does `config` have `provider`") were answering off two different state models — the path-replay timeline for the first, a one-shot structural literal scan for the second — exactly the asymmetry the review named. Fixed with `definitelyHasPath` (plus `replayPathDefinitelyHas`/`writeDefinitelyHas`/`literalDefinitelyHas`), a presence-side mirror of `replayPath`/`writeValues`/`literalValues` sharing the identical event list, position/match ordering, and capture/detach rules, but combined with AND at every branch point (a ternary/short-circuit only guarantees presence when *every* branch does — the opposite of the OR-shaped union the value side needs) and counting a write the moment it reaches the exact requested leaf regardless of whether its own value folds (presence and knowing the value are different questions). Self-caught before landing, via the existing "never guess from an unknown source" regression (`getConfig()`): a bare presence check alone can't distinguish "known shape, key absent" from "totally opaque source," and collapsing them made the resolver start guessing defaults against opaque sources — fixed by keeping the pre-existing structural `owners` (container-shape) check *alongside* the new presence check, both required, exactly mirroring how the original design already separated "is the shape known" from "does it have the key," just with the second question now timeline-aware. Verified all three reviewer-named cases, the unknown-source non-regression, and reran both of round 63's own repro batteries (the 20-case nested-member battery, the 22-case node-verified capture/alias battery) plus a full real-tree diff — zero regressions, 3015 root entries unchanged (this pattern doesn't occur in the real tree). `pytest tests/architecture` — 176 passed (up from 172: 3 LiteLLM-gate + 1 prompt-gate regression, covering the reviewer's own three cases plus the unknown-source guard); `validate_architecture.py`, `validate-docs`, and `quick-check` — 1133 passed, all green. | Commit. |
| 2026-09-03 | Round 65 — review ("Code-review (me #50)", reviewing commit `1c3cc1c4`) confirmed round 64's presence check fixed the reviewer's own false-positive case, and found the new `definitelyHasPath` was answering the wrong question in two compounding ways: (1) a real destructuring default fires when the extracted value is `undefined`, not merely when the property was never *declared* — `definitelyHasPathInner`'s `if (!path.length) return true;` treated reaching the exact leaf as proof by itself, so an explicit `config.provider = undefined;` before `const { provider = "openai" } = config` was read as "definitely present" and wrongly suppressed the default; (2) `replayPathDefinitelyHas` only ever accumulated `false → true` (`if (deterministic && contributes) certain = true;`), never resetting, so a later *deterministic* write that plainly drops the key (`config = {};` after an earlier write established it) could never undo an already-`true` verdict either. Reproduced all five reviewer-named cases against current code first (explicit `undefined`, root rebind to `{}`, nested intermediate rebind to `{}`, a conditional removal that must leave the default reachable *alongside* the original value, and the equivalent `preset`/`role` prompt case) — confirmed real against actual `node` execution for the first four (not just reasoning), and one of my own repro fixtures for the conditional case initially had a genuine bug of its own — reassigning a `const` binding, which is a compile error in real JS — self-caught by checking the resolver's `decl.mutable` gate correctly refused to record that impossible write, not a resolver bug; fixed the fixture to `let` and reverified against real `node` (`internal` when the branch isn't taken, `openai` when it is). Root-caused, not patched per-case: "does this key exist" was still the wrong question (round 64 fixed *which state model* answered it, but not *what it was asking*) — the real predicate is "is this guaranteed non-`undefined`," and the accumulation needed the value replay's own replace-on-deterministic/AND-on-conditional structure, not a monotonic OR. Fixed `definitelyHasPathInner`'s leaf case to disqualify `void x` and defer to a binding's own timeline for an identifier (the global `undefined` resolves no local declaration and falls through to `false` on its own, no special-casing needed), and changed `replayPathDefinitelyHas`'s accumulation to `certain = deterministic ? contributes : certain && contributes` — a deterministic write replaces the verdict outright (matching the value replay's `reachable = new Set(...)`), a conditional one only preserves an existing guarantee when it also holds if the write fires (matching the reviewer's own "both the executed and non-executed outcomes" framing). Verified all five reviewer cases (three now via the real gate functions directly, confirming exact offender lines), reran both of round 63's repro batteries (20-case nested-member, 22-case node-verified capture/alias) plus a full real-tree diff — zero regressions, 3015 root entries unchanged. `pytest tests/architecture` — 181 passed (up from 176: 4 LiteLLM-gate + 1 prompt-gate regression, the reviewer's own five cases); `validate_architecture.py`, `validate-docs`, and `quick-check` — 1138 passed, all green. | Commit. |
| 2026-09-03 | Round 66 — review ("Code-review (me #51)", reviewing commit `e05681ff`) confirmed round 65's "guaranteed non-`undefined`" predicate and replace/AND accumulation fixed the reviewer's own three cases, and found `definitelyHasPathInner`'s leaf case still over-generalized: `if (!ts.isIdentifier(expr)) return !path.length;` treated *any* non-identifier expression reaching the leaf as proof of a defined value — including a `CallExpression`, which can obviously return `undefined`. `const config = { provider: getProvider() }; const { provider = "openai" } = config;` was reproduced against current code first (confirmed real against actual `node` execution: when `getProvider()` returns `undefined`, `provider` is `"openai"` at runtime) — the resolver's value side can't fold the call, and the definedness side wrongly proved presence anyway, so the default was suppressed and the false-negative was silent (no offender reported at all) rather than merely wrong. The review also flagged the fix's own failure mode in advance: naively making the leaf check "conservative" by requiring both operands of `??`/`||` deserved special handling too, since `getProvider() ?? "custom"` is guaranteed non-`undefined` purely because `"custom"` is, regardless of the call — a blanket AND across both operands (right for `&&`) would have wrongly gone back to suppressing `"openai"` as a default *there* while also wrongly failing to suppress it when the fallback alone already guarantees the value, the exact "opposite false-positive" the review named. Root-caused, not patched narrowly: replaced the blanket `!ts.isIdentifier(expr)` fallback with an explicit allowlist of structurally-safe leaf kinds (`StringLiteral`, `NoSubstitutionTemplateLiteral`, `TemplateExpression`, `NumericLiteral`) plus `+` (string/numeric coercion never literally produces `undefined`, whatever the operands are) — everything else, a `CallExpression` included, now correctly returns `false` (not guaranteed) rather than guessing `true`; and gave `??`/`||` an operator-specific rule at the leaf (`path.length === 0`): the whole expression is guaranteed non-`undefined` as soon as the *right* operand alone is, since the left operand, when it's the one that wins, is by definition non-nullish (`??`)/truthy (`||`) and therefore already non-`undefined` on its own — `&&`, whose own "left wins" branch is exactly the left operand's own (possibly `undefined`) falsy value, keeps the original blanket AND, and so does `??`/`||` itself once there's a deeper path left to check (a non-nullish/truthy left operand says nothing about whether *its own* nested member exists). Verified all three reviewer-named cases (the call-result case, the equivalent prompt-role case, and the `??` fallback that must *not* over-correct into leaking `"openai"` back in) plus a fourth of my own — `&&` with a call on either side — confirming it deliberately keeps the blanket check rather than getting the `??`/`||` relaxation, all against real `node` execution and the actual gate functions. Reran every prior round's repro battery (round 63's 20-case nested-member and 22-case node-verified capture/alias batteries, round 65's 5-case undefined/rebind battery) plus a full real-tree diff — zero regressions, 3015 root entries unchanged. `pytest tests/architecture` — 185 passed (up from 181: 3 LiteLLM-gate + 1 prompt-gate regression, the reviewer's own three cases plus the `&&`-asymmetry check); `validate_architecture.py`, `validate-docs`, and `quick-check` — 1142 passed, all green. | Commit. |
| 2026-09-03 | Round 67 — review ("Code-review (me #52)", reviewing commit `bd370d66`) confirmed round 66's allowlist fix stopped a dynamic call result from suppressing a destructuring default, and found the allowlist itself was too narrow in the opposite direction: `NEVER_UNDEFINED_LEAF_KINDS` covered only string/template/numeric literals, so `null` and boolean literals fell through the fallback to `false` ("not guaranteed non-`undefined`") even though a real destructuring default fires *only* on `undefined` — never on `null`, `false`, or any other falsy-but-defined value. `const config = { provider: null }; const { provider = "openai" } = config;` was reproduced against current code first (confirmed real against actual `node` execution: `provider` is `null` at runtime, the `"openai"` default never runs) plus the equivalent boolean-literal and prompt-role cases — all three showed the resolver wrongly injecting the default as a reachable value, a false-positive that can fail architecture validation on completely safe code. Root-caused per the review's own framing (the actual predicate is "can this evaluate to `undefined`," not "is it string-shaped"): extended `NEVER_UNDEFINED_LEAF_KINDS` to the full family of atomic JS/TS primitive-literal tokens that share the exact same guarantee a string literal already had — nothing to evaluate beyond the token itself — `NullKeyword`, `TrueKeyword`, `FalseKeyword`, `BigIntLiteral`, `RegularExpressionLiteral`; left `NewExpression`/`ArrowFunction`/`FunctionExpression` and other more structurally-involved-but-still-safe shapes undocumented-but-out-of-scope, since the review named primitive literals specifically and this round's regressions cover exactly that family. Verified all three reviewer cases via both the raw resolver output and the real gate functions directly, reran every prior round's repro battery (round 63's 20-case and 22-case batteries, round 65's 5-case undefined/rebind battery, round 66's 4-case call-result/`??`/`&&` battery) plus a full real-tree diff — zero regressions, 3015 root entries unchanged. `pytest tests/architecture` — 188 passed (up from 185: 2 LiteLLM-gate + 1 prompt-gate regression, the reviewer's own three cases); `validate_architecture.py`, `validate-docs`, and `quick-check` — 1145 passed, all green. | Commit. |
| 2026-09-03 | Round 68 — review ("Code-review (team lead #4)", re-reviewing commit `3ffb25b5`) reported three blocking boundary bypasses. Reproduced all three against current code first, via two parallel Explore agents, before any fix: (1) the JS/TS prompt gate's key-scanning regexes only matched a `role:`/`instructions:` object-literal key or ES2015 shorthand — a staged request object built via member assignment (`req.role = SYSTEM_ROLE;`, `request["role"] = PROMPT;`) hit neither shape, confirmed `offenders == []` against `check_prompts_inside_extensions` for all three of the reviewer's repro forms even though the shared JS resolver could already resolve the RHS. (2) the Python endpoint gate's `_add_module_level_constants` walked `tree.body` directly with zero `ast.If` handling anywhere in the file, so a module-level constant reassigned inside an `if` was invisible — already logged as deliberately-deferred Follow-up debt since ~round 45. (3) `module_import_aliases`'s own docstring named bare `import pkg.paths` (no `as`) as deliberately out of scope, confirmed unresolved: alias registration requires `alias.asname`, and the resolver's one-hop-only `ast.Attribute` match can't structurally reach a two-level `pkg.paths.NAME` chain either — also pre-existing, documented debt. Fixed (2) and (3) at the shared-model level in `static_string_resolution.py`, not the one endpoint-gate call site named, since both live in the module the LiteLLM and provider-host gates also consume: `PythonModules.constants` moved from one `str` per name to a `frozenset[str]` of every reachable value (mirroring the JS resolver's own existing per-position model, which the module's own docstring already promised for both languages) — a deterministic write replaces the set, a write inside an `ast.If`'s `body`/`orelse` (recursed into, covering `elif`/nested `if`) unions into it instead, since the branch may or may not run; `python_string_value` renamed `python_expr_values` and its three call sites (plus `python_string_values`'s tree-walker) moved to the set-returning shape together, deliberately keeping `+`/f-string folding restricted to single-valued operands rather than building a Cartesian product nothing here needs yet. Bare dotted imports generalized (not hardcoded to the reviewer's exact two-level example) via a new `bare_import_roots: dict[Path, set[str]]` (the set of local top-level names a bare `import a.b...` binds) plus an `_attribute_chain` walk resolving any depth of dotted chain rooted at one of those names against the existing `module_paths` suffix index — `import pkg.paths.sub; x.paths.sub.NAME` resolves the same way `import pkg.paths; pkg.paths.NAME` does, for free. Fixed (1) with two more regexes (`_JS_DOT_ASSIGN_KEY_RE`/`_JS_BRACKET_ASSIGN_KEY_RE`) reusing the exact `js_string_expr_at` lookup the colon-form branch already calls — no `js_scope_resolver.mjs` change needed, since the resolver could already fold the RHS; `(?!=)` after the `=` excludes `==`/`===`, verified with a negative-control regression. New regressions: 4 in the JS prompt gate (dot-assignment `role`, bracket-assignment `role`, dot-assignment `instructions`, `===`-comparison non-match) and 3 in the Python endpoint gate (conditional reassignment detected, safe-branches-only non-false-positive, bare dotted import detected). Reran the full accumulated `pytest tests/architecture` suite (covers every round 63-67 regression already promoted into it) plus the real-tree-backed `test_no_product_specific_endpoint_paths`/LiteLLM/provider-host tests against the actual monorepo — zero regressions. `pytest tests/architecture` — 195 passed (up from 188: 4 prompt-gate + 3 endpoint-gate regressions); `validate_architecture.py` and `validate-docs` — clean; `quick-check` (clean `scripts/agent/node_modules`) — 1152 passed (up from 1145), all green. | Commit. |
| 2026-09-03 | Round 69 — review ("Code-review (me #53)", reviewing commit `0ed90671`) confirmed the three round-68 findings are fixed, and found two follow-on gaps in round 68's own new Python conditional-value model, both reproduced against current code first. (1) `python_expr_values`'s `+`/f-string folding required every operand/part to resolve to exactly one value each — round 68's own docstring had named this a deliberate scope cut, and the review showed it recreates the exact bypass round 68 closed: `PREFIX = "/safe"; if enabled: PREFIX = "/proposal_ai"; PATH = PREFIX + "/status"` (and the f-string equivalent, `f"{PREFIX}/status"`, with no intermediate `PATH` at all) resolved to nothing once `PREFIX` had more than one reachable value, so `_add_module_level_constants` never recorded `PATH` and the forbidden route went unreported — confirmed `offenders == []` for both forms. Fixed with `_combine`, a direct, same-cap port of `js_scope_resolver.mjs`'s own `combine()` (`_MAX_COMBINATIONS = 256`, same reasoning — a real hardcode fed by more than a couple of combined conditional branches is vanishingly unlikely and would very likely also be reachable through a simpler branch elsewhere), used by both `BinOp(+)` and `JoinedStr`. (2) a genuine branch-state correctness bug, not merely a scope gap: each `ast.If` branch's write had been unioned straight into the one live, already-accumulated constants dict, so a value written before the `if` stayed reachable forever even once every runtime path through the `if`/`else` unconditionally overwrote it — `PATH = "/proposal_ai/status"; if flag: PATH = "/safe-a" else: PATH = "/safe-b"` kept the forbidden initial value falsely reachable and produced a false-positive offender on code that's actually safe at runtime; confirmed via a direct repro against the real gate. Fixed by evaluating each branch from an independent copy of the state as of just before the `if` (not the shared live dict) and joining the two branches' *resulting* states by union only once both are done, rather than unioning each write in as it's found — the join point moved from "every write" to "the two branch outcomes." New regressions: conditional constant composed via `+`, the same via f-string, both branches overwriting a forbidden value drops it at the join, and sequential writes inside one branch leave only the final write reaching the join. Reran the full `pytest tests/architecture` suite (covers every prior round's regression already promoted into it) — zero regressions. `pytest tests/architecture` — 199 passed (up from 195: 4 endpoint-gate regressions); `validate_architecture.py` and `validate-docs` — clean; `quick-check` (clean `scripts/agent/node_modules`) — 1156 passed (up from 1152), all green. | Commit. |
| 2026-09-03 | Round 70 — review ("Code-review (me #54)", reviewing commit `d1874378`) confirmed both round 69 fixes hold, and found `_add_module_level_constants` still only ever matched `ast.Assign`/`ast.AnnAssign` — `ast.AugAssign` fell straight through the `else: continue`, unmodeled, so `PATH = "/proposal"; PATH += "_ai/status"` left the resolver holding only the stale `"/proposal"` even though both operands are literal and the runtime result is fully deterministic; reproduced against current code first (`offenders == []`). Fixed in the shared resolver rather than the endpoint gate, per the review's own explicit framing (`static_string_resolution.py` is shared by multiple gates): `ast.AugAssign` with `ast.Add` on an `ast.Name` target is now handled alongside `ast.Assign`/`ast.AnnAssign`, replacing the name's reachable set with `_combine([previous, rhs])` — the same Cartesian-product helper round 69 introduced, so it composes correctly with a name that already carries more than one reachable value, and gets round 69's entry-state/join machinery for free the moment it appears inside an `ast.If` branch, with zero extra branch-state code. New regressions: the reviewer's own deterministic `+=` case, and a conditional `+=` reachable on only one branch (checking both the original and the augmented result survive to the join, per the review's own explicit ask). Reran the full `pytest tests/architecture` suite — zero regressions. `pytest tests/architecture` — 201 passed (up from 199: 2 endpoint-gate regressions); `validate_architecture.py` and `validate-docs` — clean; `quick-check` (clean `scripts/agent/node_modules`) — 1158 passed (up from 1156), all green. | Commit. |
...
- Round 63 replaced the round 58–62 per-property member model with one path model
  ...
  overwrite), and ternary/short-circuit expression values (both branches reachable). Round 64
  closed the last place that still bypassed the path model: `defaultedValues`'s own absence check
  now asks the same timeline (`definitelyHasPath`) the value read does, so a deterministic member
  write recorded anywhere on the path — not just the source's original literal shape — correctly
  suppresses a destructuring default, while a merely conditional one still leaves both the found
  value and the default reachable; the existing structural "is the shape known at all" check stays
  alongside it unchanged, so a destructuring off a genuinely opaque source (a function call, an
  unresolvable import) still never guesses a default. Round 65 corrected what that timeline was
  actually proving: a destructuring default fires on `undefined`, not on mere absence, so the
  presence check now asks "guaranteed non-`undefined`" (an explicit `= undefined` or `void x`
  disqualifies; a deterministic write that removes the key, e.g. a root/intermediate rebind to
  `{}`, correctly un-proves an earlier guarantee instead of leaving it stuck `true`) using the same
  replace-on-deterministic/AND-on-conditional accumulation the value replay itself uses, rather
  than a monotonic OR. Round 66 narrowed the leaf case of that check: "any self-contained
  expression counts" wrongly included a `CallExpression` (a function can return `undefined`), so a
  property initialized from a call result no longer wrongly suppresses a destructuring default;
  only an explicit allowlist of structurally-safe kinds (string/template/numeric literals, `+`, and
  the identifier/container paths already handled) counts, with `??`/`||` getting an operator-
  specific rule — the whole expression is guaranteed non-`undefined` as soon as the *right* operand
  alone is, regardless of the left — that `&&` deliberately does not share (its own "left wins"
  branch is `a`'s own possibly-`undefined` falsy value, so it keeps the blanket AND). Round 67
  found the allowlist itself was too narrow the *other* way: a destructuring default fires only on
  `undefined`, never `null` or any other falsy-but-defined value, so `null`/`false` fell through to
  "not guaranteed" and wrongly injected a default that could never run at runtime — extended the
  allowlist to every atomic JS/TS primitive-literal token (`null`, `true`, `false`, BigInt, regex).
  Still open,
...
- Round 68 closed the two Python-resolver gaps every prior bullet above had been carrying forward
  as open: `_add_module_level_constants` now recurses into `ast.If` branches (unioning a
  conditionally-reachable value into the name's `frozenset[str]` rather than skipping the branch
  entirely), and a bare `import pkg.paths` (no `as`) now resolves through a new
  `bare_import_roots`/`_attribute_chain` mechanism generalized to any dotted depth, not just the
  reviewer's two-level example. Newly scoped out, deliberately, by this same round's fix: only
  `ast.If` is modeled for Python's conditional constants — `try`/`for`/`while`/`with` bodies stay
  invisible (same "safe in direction, never a wrong value" property the old gap had); `+`
  concatenation and f-strings still only fold when every operand/part resolves to exactly one
  value each, so a conditionally-reachable name combined with either doesn't get a Cartesian-
  product expansion (nothing in this repo needs it yet); and the JS/TS prompt gate's new dot-
  /bracket-assignment detection (`req.role = X`) only matches a plain `=` — compound assignment
  (`??=`, `+=`) isn't recognized. All other items in the round 62-67 list directly above are
  unchanged by this round.
- Round 69 closed the `+`/f-string single-valued-operand cut round 68 had just scoped out (the
  review showed it recreated the exact bypass round 68 closed, the moment a conditionally-
  reachable name was composed with a literal): `python_expr_values`'s `BinOp(+)`/`JoinedStr`
  folding now takes the full Cartesian product of every operand/part's reachable set via a new
  `_combine` helper, capped at `_MAX_COMBINATIONS = 256` — a direct, same-cap port of
  `js_scope_resolver.mjs`'s own `combine()`, not a new bound invented for the Python side. Also
  fixed a genuine correctness bug (not a scope gap) round 68's own conditional model had
  introduced: an `ast.If` branch's write used to union straight into the one live, running state,
  so a value from before the `if` stayed reachable forever even once both branches
  unconditionally overwrote it; `_add_module_level_constants` now evaluates each branch from an
  independent copy of the entry state and unions only the two branches' *resulting* states at the
  join. Still open, unchanged: only `ast.If` is modeled (`try`/`for`/`while`/`with` stay
  invisible); compound assignment in the JS/TS prompt gate's dot/bracket forms; and every item in
  the round 62-67 list two bullets above that round 68 didn't touch.
- Round 70 closed a gap never previously catalogued here (found by the review, not a documented
  deliberate cut): `ast.AugAssign` (`PATH += "x"`) was skipped entirely by
  `_add_module_level_constants`, so a deterministic route path built with `+=` kept the stale
  pre-assignment value. Now handled (`+` only) alongside `ast.Assign`/`ast.AnnAssign`, composing
  via the same `_combine` Cartesian product round 69 introduced, so it also works correctly inside
  an `ast.If` branch with no extra code. Still open, unchanged: only `ast.If` is modeled for
  Python's conditional constants; only `+` augmented assignment is handled (`-=`/`*=`/etc. aren't
  meaningful for string route paths and stay unmodeled); compound assignment in the JS/TS prompt
  gate's dot/bracket forms; and every item in the round 62-67 list three bullets above.
