...
- Round 43 replaced the entire JS/TS side of the shared resolver with a real TypeScript-compiler
  ...
  `export default` isn't followed; and only bare `${NAME}` template holes fold. Upgrade path for
  all of them, if ever needed, is extending the same real-AST script rather than adding more
  character-level heuristics. Python's own module-constant resolution has a parallel, currently
