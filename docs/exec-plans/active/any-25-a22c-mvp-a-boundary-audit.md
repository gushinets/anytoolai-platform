...
- Next action: none — implementation, three internal code-review rounds, and twenty-three GitHub
  ...
  concatenation bypassing the LiteLLM scanner in both Python and JS/TS — and explicitly scoped out
  three findings against pre-existing, PR-untouched test files or asking for a large non-blocking
  fixture suite, logged as follow-up debt instead — see those entries); move to `completed/` once
  merged.
- Blocker: none
...
| 2026-09-01 | Left three of the team-lead review's six findings unfixed: strengthening `test_no_direct_provider_calls_outside_gateway.py` (JS/TS provider-package/raw-HTTP-host enforcement) and `test_no_prompts_inside_extensions.py` (structural system-role message-shape detection), and adding permanent parameterized temp-tree regression fixtures for the two new scanners. | Both named test files are pre-existing and untouched by this PR's diff — extending them is real, valuable boundary-hardening work, but a materially larger and separate effort from ANY-25's own two-new-scanners/two-docs scope, not an implicit part of this audit; the fixture-suite request is legitimate but non-blocking and large on top of an already very large diff. All three logged as follow-up debt for a dedicated ticket instead of expanding this PR further. |
...
- `test_no_direct_provider_calls_outside_gateway.py` is Python-import-only: no JS/TS provider SDK
  package-specifier check (`@anthropic-ai/sdk`, `openai`, `@google/genai`, `cohere-ai`,
  `@mistralai/mistralai`, ...) and no raw-HTTP-to-provider-host check (`fetch("https://api.openai.com/...")`)
  exist at all today. Real gap, flagged by the team-lead review (round 24) — out of scope for this
  PR since the file predates and isn't touched by it.
- `test_no_prompts_inside_extensions.py` only checks for the literal tokens `system prompt`,
  `prompt_ref`, `provider_policy_ref`; a structural system-role message shape
  (`{ role: "system", content: "..." }`) passes unchanged. Real gap, flagged by the same round-24
  review, same out-of-scope reasoning.
- Both new scanners (`test_no_product_specific_endpoints.py`,
  `test_litellm_model_strings_stay_in_provider_config.py`) are only exercised against the real
  repository tree today; a future refactor could regress a fixed edge case silently until
  production code happens to contain the affected syntax. Round 24's review requested permanent
  parameterized temp-tree regression fixtures for both — legitimate, non-blocking, deferred as a
  separate follow-up given the diff's existing size.
