...
- Next action: none — implementation, three internal code-review rounds, and twenty-three GitHub
  ...
  two request-key regexes comment-tolerant; move to `completed/` once merged.
- Blocker: none
...
- Known, documented ceilings of the shared resolver (`ponytail:` comments in
  `static_string_resolution.py`): JS/TS `var`'s real function scope is approximated as
  block scope (narrower — can only miss an in-scope `var`, never resolve one that isn't), a bare
  assignment anywhere in a declaration's own scope invalidates it even when it actually targets a
  more-nested shadowing declaration of the same name, only bare `${NAME}` template holes fold,
  only relative named imports propagate, and a bare Python `import a.b` (no `as`) isn't followed.
  Upgrade path for all of them is a real JS/TS parser; none is a bypass a reviewer has
  demonstrated. (Round 34's "one flat namespace, no scope tracking" ceiling was closed in round
  36 — the JS/TS resolver is now lexically scope- and mutation-aware.)
