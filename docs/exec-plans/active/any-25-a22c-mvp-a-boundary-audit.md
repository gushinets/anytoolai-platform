...
- Next action: none — implementation, three internal code-review rounds, and twenty-three GitHub
  ...
  fold from same-line-only to whole-file (multi-line and grouping-paren shapes); only the
  temp-tree-regression-fixture ask remains deferred as follow-up debt — see those entries); move
  to `completed/` once merged.
- Blocker: none
