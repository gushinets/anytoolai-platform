...
- Next action: none — implementation, three internal code-review rounds, and twenty-three GitHub
  ...
  whitespace between a guard/`=>` and a body that starts on the next line; move to `completed/`
  once merged.
- Blocker: none
...
- Known, documented ceilings of the shared resolver (`ponytail:` comments in
  `static_string_resolution.py`): JS/TS `var`'s real function scope is approximated as block
  scope (narrower — can only miss an in-scope `var`, never resolve one that isn't), a `do` loop's
  braceless body (terminated by a trailing `while (...)`, not a `;`) isn't recognized as
  conditional, a comma inside an object literal's `{ ... }` isn't recognized as a sibling-property
  boundary the way one inside `(...)`/`[...]` is (a `{`/`}` can't be reliably told apart from an
  ordinary statement block, where a comma has entirely different meaning, without a real parser —
  an arrow assigned as one object property's value, immediately followed by a comma and another
  property reading an outer same-named binding, is the one shape this can still miss), a newline
  is always treated as unconditional ASI even when the expression genuinely continues onto the
  next line via a trailing operator with no enclosing brackets at all (`const x = a +\n  b`, rare
  in practice and typically avoided with explicit parens anyway), a Cartesian-product combine
  (`_combine`) caps at 256 possible values per expression rather than doing real path-sensitive
  analysis (a real hardcode fed by more than a couple of conditional branches in one expression is
  vanishingly unlikely, and would very likely also be reachable through a simpler branch
  elsewhere), a reassignment reachable from a declaration's own scope can attach to the wrong
  sibling shadowing declaration of the same name (over-attaches, doesn't under-detect), only bare
  `${NAME}` template holes fold, only relative named imports propagate, and a bare Python `import
  a.b` (no `as`) isn't followed. Upgrade path for all of them is a real JS/TS parser; none is a
  bypass a reviewer has demonstrated. Python's own module-constant resolution has a parallel,
  currently unreported gap: `_add_module_level_constants` only scans `tree.body`, so an assignment
  inside a conditional branch (`if cond: NAME = "x"`) is invisible rather than wrongly resolved —
  safe in direction (never a wrong value) but, unlike the JS/TS resolver as of round 38, doesn't
  track a conditionally-reachable value as *also* reachable; not fixed here since no review round
  has raised it for Python. (Round 34's "one flat namespace, no scope tracking" ceiling was closed
  in round 36; round 36's own "any reassignment invalidates the binding" ceiling was closed in
  round 37; round 37's own "every write, including a conditional one, deterministically overrides"
  ceiling was closed in round 38; round 38's own "conditional-vs-deterministic decided by `{}`
  depth alone" ceiling was closed in round 39 for braceless `if`/`for`/`while`/`else` bodies; round
  39's own "concise-body arrow parameter defaults unmodeled" ceiling was closed in round 40; round
  40's own "a concise arrow's virtual scope only closes at `;`/`}`, not at a comma/bracket-close/
  ASI-newline the way a real expression actually ends" ceiling was closed in round 41 — the
  resolver now tracks the full set of statically-known values reachable at each use site,
  regardless of whether the control flow or function body producing them uses braces, and bounds
  a concise arrow's parameter scope to its actual expression rather than the surrounding
  statement.)
