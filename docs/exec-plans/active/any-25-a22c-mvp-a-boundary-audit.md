...
- Next action: none — implementation, three internal code-review rounds, and twenty-three GitHub
  ...
  (`if (a) if (b) x = 1;`) and `for`/`while` guard clauses' own internal semicolons; move to
...
- Known, documented ceilings of the shared resolver (`ponytail:` comments in
  ...
  (over-attaches, doesn't under-detect), a concise-body arrow parameter default (`(role = "x") =>
  role`, no `{ ... }` block to anchor the parameter's scope to) isn't modeled, only bare `${NAME}`
  template holes fold, only relative named imports propagate, and a bare Python `import a.b` (no
  `as`) isn't followed. Upgrade path for all of them is a real JS/TS parser; none is a bypass a
  reviewer has demonstrated. Python's own module-constant resolution has a parallel, currently
  ...
  direction (never a wrong value) but, unlike the JS/TS resolver as of round 38, doesn't track a
  ...
  depth alone" ceiling was closed in round 39 for braceless `if`/`for`/`while`/`else` bodies — the
  resolver now tracks the full set of statically-known values reachable at each use site
  regardless of whether the control flow producing them uses braces.)
