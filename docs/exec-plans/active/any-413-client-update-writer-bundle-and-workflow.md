...
- Next action: PR open, CI green; awaiting merge.
- Blocker: none
...
3. **Workflow-level input schemas are product-owned and decoupled from the atom's own input
   ...
   `tone` + optional `constraints`); mode-specific voice lives in the product-owned prompt text,
   not in the input schema. `intent`/`objective` framing is a workflow `literal:` value for Update
   and PrepaidRequest (both pursue a fixed goal the product defines), but ReplyDraft maps `intent`
   from `scenario.input.reply_goal` instead — a reply must state the caller's actual answer (a
   real fact: a date, a decision), which can't be a fixed literal.
...
6. **No `quotas.yaml`/`handoffs.yaml`** — the ticket names no quota policy or handoff target, and
   both are optional to the loader when `product.yaml` sets no `quota_policy_ref`. *(For quotas, see
   ANY-414: its PR adds a guest quota whose parameters are pending a product decision --
...
| 2026-09-10 | Rounds 4-7 (grounding hardening): fixed PrepaidRequest's billing amount/due date never actually reaching the client message end-to-end; removed invented causal/gating claims and absence-of-information claims from fixtures; stopped tightening a stated due date to an earlier one; preserved upstream urgency wording and fixed a wrong-party CTA phrasing ("confirm receipt" when the client is the one sending payment, not receiving it); hardened all 3 input schemas against whitespace-only/trailing-newline content and language-code injection. Round 7 added the missing scope deliverable: `renderer_contract.yaml` pinning `text` as the canonical copy-ready field and `call_to_action`'s composition rule, plus a regression test. | Await round 8 or merge |
| 2026-09-10 | Rounds 8-9: strengthened the renderer-contract regression test to actually assert `call_to_action_composition` (it asserted everything else the contract defines except the one field its own name promised to pin) and to verify `canonical_field`/`call_to_action_field` exist as real properties on the referenced output schema. | Merge, pending further review |
| 2026-09-14 | Code review round "team lead #1": fixed two real blockers — PrepaidRequest's persuasive-text step never told the model how to honor `constraints.language/length/format` even though the workflow maps them into its input (copied the canonical A06 prompt's language/length/format rules into the product prompt); the reply step's no-invention rule covered only causal/gating claims, not invented payment method/link/reference/account details (extended it to the whole output). Also fixed: ReplyDraft's `call_to_action` rule allowed turning a plainly-stated fact into a question the client must confirm (the happy fixture did exactly this — "Wednesday" became "let me know if Wednesday works"); tightened the rule and dropped the now-rule-violating `call_to_action` from that fixture. Replaced the product test's hand-maintained provider/model-string denylist with `validate_architecture.py`'s own `LLM_PROVIDER_IMPORTS` (matching ProposalAI's existing pattern) plus model-string prefixes, closing a drift risk. Synced this exec plan's design decision 3 (ReplyDraft's `intent` is `scenario.input.reply_goal`, not a literal like the other two modes) and Status metadata with current state. | Await further review or merge |
| 2026-09-14 | Round "me #10": added a prompt-text regression test pinning that the persuasive-text prompt actually documents `constraints.language/length/format` (the deterministic e2e tests only run against fixed fake-provider fixtures, so only a test reading the prompt template directly can catch a future regression there). | Await further review or merge |
| 2026-09-18 | Rounds "team lead #2" and "me #11": pinned the prepaid workflow's `constraints.language/.length/.format` mapping source paths with a regression test (previously untested — a source typo could silently drop them for a live provider while the fake-provider e2e stayed green); fixed the prepaid reply fixtures (happy + weak) repeating the persuasive step's first sentence verbatim, against the reply prompt's own "without repeating it verbatim" rule — then, in "me #11", caught that the weak fixture's fix had gone too far and dropped the "what the payment is for" context entirely, so re-added it as a paraphrase instead of a drop. | Await further review or merge |
| 2026-09-18 | Round "team lead #3": fixed a real blocker — the happy PrepaidRequest reply fixture had also lost A06's grounded framing (approved design phase wrapping up, phase 2 starting) when the verbatim-repeat fix removed it outright; restored it as a paraphrase. Also fixed the reply prompt's `call_to_action` rule always restating the payment amount, which duplicated the ask already stated in `text` once the renderer composed them together — narrowed the CTA rule to the follow-up acknowledgement only, updated both fixtures' `call_to_action` and the E2E expected output to match. | Await further review or merge |
| 2026-09-18 | Round "xhigh #5": fixed the 4 Client Update Writer prompts' stale "`text` must contain no markup" plain_text wording — ANY-502 (`2becc966`) already relaxed the shared A06/A07 cross-validators to allow CommonMark lists in `plain_text`, and `kernel_demo`'s own prompts were updated to match, but these 4 product prompts weren't. Added a regression test pinning the reworded `call_to_action` rule from "team lead #3" (no prior test covered it) and a test pinning each mode's `constraints: '?scenario.input.constraints'` compose_reply mapping (previously only the persuasive-text step's per-field mapping had one). Added an E2E assertion that the final PrepaidRequest reply's `text` literally contains the scenario input's own `billing_context.amount`/`.due_date` — decoupled from the fixture's hardcoded expected string, so a future prompt/fixture edit that silently drops either value now fails independently of whatever the fixture itself says, closing the gap that let several earlier rounds patch the same amount/due-date-propagation risk without a guard against it recurring a fourth time. Reconfirmed (not changed): `client_update_writer` still ships no `quota_policy_ref`/`quotas.yaml` — this is Design decision 6 below, a deliberate choice recorded when the ticket was scoped, not a regression; both are optional to the config loader when unset. | Await further review or merge |
