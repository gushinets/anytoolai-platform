...
| 2026-09-10 | Addressed a third human PR review round (PR #113): no remaining blocking (P1) findings -- the closed-vocabulary fix from the prior round resolved the privacy blocker. One non-blocking P2: `_load_analytics()` accepted any `client_event_properties` key as long as its value was a list of strings, so a typo like `gap_categry` would pass `validate-configs` silently while `ClientEventService` (which only ever looks up `mode`/`gap_category` by exact name) never consults it -- leaving the real key's vocabulary empty with no diagnostic pointing at the typo. Restricted `client_event_properties` keys to `CLIENT_EVENT_CATEGORICAL_PROPERTY_KEYS` (the string-typed subset of `CLIENT_EVENT_PROPERTY_TYPES` -- excludes `field_count`, which is int-typed and has no vocabulary concept), imported from `events/client_events.py` into `config/loader.py` (verified no circular-import risk first: neither module's dependency chain imports back into the other; this also follows the loader's own existing precedent of importing domain enums like `FrontendType` for config validation). Added loader tests for an unsupported key (`gap_categry`) and for `field_count` specifically (a real property name, but not a categorical one). `postgresql-check`, the new loader tests, and `frontend-check` all green. | None -- ready for user review/commit. |
