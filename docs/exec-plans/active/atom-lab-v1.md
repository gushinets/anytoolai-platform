...
| 2026-09-25 | Closed the three post-merge ANY-466 review findings: an open History detail now recomputes model availability on every catalog load/refresh; history rows display `preset_id` with the version; and ambiguous version writes retain the frozen payload/base so same-identity reconciliation can unlock a safe retry, confirm an exact committed version, or surface a concurrent conflict without discarding either the submitted draft or later local edits. Verified 51 Node tests, all 82 Atom Lab Chromium journeys, ESLint, architecture tests, `git diff --check`, canonical `quick-check` (1885 passed), and a clean independent diff re-review. Canonical `frontend-check` passed install/lint/typecheck and the feature-owned 82 journeys, then the unrelated web-mirror Vitest suite remained incompatible with local Node 26 `localStorage`; GitHub's supported Node gate is the authoritative rerun. | Commit and push the follow-up, open the post-merge PR, reply to the three inline comments with its commit/coverage, and monitor replacement CI. |
