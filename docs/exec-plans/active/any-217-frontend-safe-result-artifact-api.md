...
- [x] Add `anytoolai_platform_core/results/service.py` (`ResultService`,
  ...
  - [x] Added `model_ref` to the denylist after a review noted `config.loader` already treats it
        as a `FORBIDDEN_RAW_LLM_FIELDS` entry alongside `provider_policy_ref`/`prompt_ref` (both
        already listed here), so an open-schema workflow output containing it would otherwise
...
| 2026-08-09 | Added `model_ref` to the denylist | A review noted `config.loader.FORBIDDEN_RAW_LLM_FIELDS` (the config-authoring-time raw-LLM-field guard) already treats `model_ref` as forbidden, and `prompt_ref`/`provider_policy_ref` from that same source list were already mirrored here, so its absence was an inconsistency rather than a deliberate exclusion. The rest of that source list (`temperature`, `top_p`, `seed`, `stop`, `stream`, `tools`, ...) is deliberately not mirrored -- those are generic words with real domain-field collision risk, unlike the specific `*_ref`/`*_id` identifier names |
...
| 2026-08-09 | Replaced the camelCase-boundary regex with separator-stripped canonical-form equality (`_canonical_key`), closing the fully-uppercase and nested-acronym-PascalCase bypass classes in one change instead of another regex rule; consolidated 4 near-duplicate key-variant tests into one parametrized test and added the new bypass spellings as cases; corrected the frontend-boundaries.md claim that no longer held; declined to memoize `_canonical_key` (would cache on unbounded request-controlled key strings) | Address review noting `model_ref` (a `config.loader.FORBIDDEN_RAW_LLM_FIELDS` entry) was missing from the results denylist |
| 2026-08-09 | Added `model_ref` to `_FORBIDDEN_OUTPUT_KEYS` and regression coverage for `model_ref`/`model-ref`/`modelRef` | None; ready for merge |
