...
is emitted when the backend rejects a scenario start because the guest/product quota is exhausted.
These events do not depend on frontend clicks, workflow success, provider calls, validation retries,
transport retries, PydanticAI telemetry, or LiteLLM telemetry.
