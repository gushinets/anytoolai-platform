...
- `packages/backend/product-platforms/freelancer-suite/` already loads as a real `ProductBundle`
  ...
  (the loader's required-evidence suite, proven against a test-only fixture bundle). As of
  `ANY-228` `FreelancerSuiteBundle.config_roots()` returns four real product roots: ProposalAI
  (`ANY-227`), Client Update Writer (`ANY-413`), Brief Decoder (`ANY-232`), and Acceptance Builder
  (`ANY-228`); see the package's own README for the full 5-product release order (`ANY-452`). Use `self._package_dir()`
