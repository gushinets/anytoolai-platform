...
- `packages/backend/product-platforms/freelancer-suite/` already loads as a real `ProductBundle`
  ...
  proven against a test-only fixture bundle). As of `ANY-227` (`B02a`) `FreelancerSuiteBundle.config_roots()`
  returns the ProposalAI product root, the first real one; see the package's own README for the
  full 6-product roadmap order. Use `self._package_dir()`
