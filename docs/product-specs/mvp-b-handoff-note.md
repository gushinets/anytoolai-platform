...
- `packages/backend/product-platforms/freelancer-suite/` already loads as a real `ProductBundle`
  ...
  `ANY-232` `FreelancerSuiteBundle.config_roots()` returns three real product roots: ProposalAI
  (`ANY-227`), Client Update Writer (`ANY-413`), and Brief Decoder (`ANY-232`); see the package's
  own README for the full 5-product release order (`ANY-452`). Use `self._package_dir()`
